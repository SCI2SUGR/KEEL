 ...
 * It is improved by using a 3-NN classifier between instances which are very
 * near
 * 
...
public class DeepsNN extends LazyAlgorithm{
	...
	protected int evaluate(double example[]) {
		...
		for(int i=0;i<trainData.length&&inside<3;i++){
			
			if(checkInside(example,trainData[i])){
		...
		if(inside>0){
			...
			if(inside>2){
				...
				for (int i=0; i<trainData.length; i++) {
				    ...
				    dist = euclideanDistance(trainData[i],example);
				...
				//we have check all the instances... let's see what's the most present class
				...
				for(int i=0;i<trainData.length;i++){
					...
					if(dist<min&&dist>0.0){
						min=dist;
						output=trainOutput[i];
					}
				}
			}
		
		}else{
			output=evaluateDeeps(example);
		}
	...
	/** 
	 * Checks if a given train instance is in the Alpha-Neighborhood
	 ...
	 * 
	 */
	private boolean checkInside(double example[],double instance[]){
		
		boolean inside=true;
	
		for(int i=0;i<example.length&&inside;i++){
			if(Math.abs(example[i]-instance[i])>ALPHA){
				inside=false;
			}
		}
	 ...
	 * 
	 */
	protected int evaluateDeeps (double example[]) {
