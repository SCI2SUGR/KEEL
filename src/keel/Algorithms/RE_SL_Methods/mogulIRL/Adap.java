...
public void CriteriosReglas (double [] cromosoma) {
 ...
 for (i=0; i<tabla.long_tabla; i++) {
   ...
   if (tabla.datos[i].cubierto==0) {
     frec_acumulada+=RCE;
     if (RCE>=omega) { /* If the example has a cover higher than omega*/
 ...
 if (tipo_fitness==2) {
   ...
   for (i=0; i<tabla.long_tabla; i++) {
     ...
     if (n_ejem_ya_cub <= umbral)  PC = 1.0;
        ...
        public double eval (double [] cromosoma, int aplicar_ee) {
                if (tipo_fitness == 1)  return (eval_mulmodal(cromosoma, aplicar_ee));
                else  return (eval_criterios (cromosoma, aplicar_ee));
        }


        /** Fitness with penalty by niches */
        public double eval_mulmodal (double [] cromosoma, int aplicar_ee) {
                double fitness;
                ...
                if (aplicar_ee == 1)  fitness *= LNIR (cromosoma);
        ...
        public double eval_criterios (double [] cromosoma, int aplicar_ee) {
                double fitness;
