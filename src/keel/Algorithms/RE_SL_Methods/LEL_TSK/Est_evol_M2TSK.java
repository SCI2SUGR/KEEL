...
class Est_evol_M2TSK {
	...
	public Est_evol_M2TSK (BaseR_TSK base_r, Adap_M2TSK fun, MiDataset t, int n_gen, int m, int l, int n_sigm, int n_alf, int Omeg_x, int Omeg_sigma, int Omeg_alfa, int Delt_x, int Delt_sigma, int Delt_alfa) {
		int i;
		...
		Tau_0 = 1.0 / Math.sqrt (2.0 * tabla.n_variables);
		Tau = 1.0 / Math.sqrt (2.0 * Math.sqrt(tabla.n_variables));
