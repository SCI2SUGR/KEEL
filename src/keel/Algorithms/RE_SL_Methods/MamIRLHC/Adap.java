...
class Adap {
	...
	public Adap (MiDataset training, BaseR base, double valor_omega, double valor_k, int fitness, int nichos) {
		tabla = training;
		base_reglas = base;
...
/* ------------------------- Criterios de reglas -------------------------- */
	...
	public double ReglaCubreEjemplo (double [] cromosoma, double [] ejem) {
		...
		for(i=0; i<tabla.n_var_estado; i++)
			if (grado_pertenencia[i] < minimo_ant)  minimo_ant = grado_pertenencia[i];

		if (minimo_ant>0)  EmparejaAnt = 1;

		for(i=tabla.n_var_estado; i<tabla.n_variables; i++)
			if (grado_pertenencia[i] < minimo_con)  minimo_con = grado_pertenencia[i];
...
/* --------------- Criterios especificos de la aplicacion ----------------- */
	...
	public void Error_tst (MiDataset tabla_tst) {
		...
		for (j=0,suma1=suma2= 0.0; j < tabla_tst.long_tabla; j++) {
			...
			suma1 += 0.5 * Math.pow (tabla_tst.datos[j].ejemplo[tabla_tst.n_var_estado]-fuerza,2.0);
...
/* ---------------------------- Funcion fitness --------------------------- */
	...
	public double eval (double [] cromosoma) {
		if (tipo_fitness == 1)  return (eval_mulmodal(cromosoma));
		else  return (eval_criterios (cromosoma));
	}
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
	...
	public double NIR2 (double [] cromosoma) {
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			/* Punto izquierdo */
			for (j=0; j<tabla.n_variables; j++)  puntos[j] = base_reglas.BaseReglas[i][j].x0;

			cubr_act[0] = ReglaCubreEjemplo (cromosoma, puntos);
			if (cubr_act[0] > max_cubr)   max_cubr = cubr_act[0];

			/* Centro */
			for (j=0; j<tabla.n_variables; j++)   puntos[j] = base_reglas.BaseReglas[i][j].x1;

			cubr_act[1] = ReglaCubreEjemplo (cromosoma, puntos);
			if (cubr_act[1] > max_cubr)  max_cubr = cubr_act[1];

			/* Punto derecho */
			for (j=0; j < tabla.n_variables; j++)  puntos[j] = base_reglas.BaseReglas[i][j].x3;
	...
	public double NIR3 (double [] cromosoma) {
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			...
			for (j=0; j<tabla.n_variables; j++) {
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)+1], base_reglas.BaseReglas[i][j]);
	...
	public double NIR4 (double [] cromosoma) {
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			...
			for (j=0; j<tabla.n_variables; j++) {
				/* Punto izquierdo */
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)], base_reglas.BaseReglas[i][j]);
				...
				/* Punto derecho */
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)+2], base_reglas.BaseReglas[i][j]);
