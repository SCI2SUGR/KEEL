...
class AG  {
	...
	public AG (int n_poblacion, int n_var, double cruce, double mutacion, double valor_a, double valor_b, Adap funcion) {
		int i;
	...
	public void Initialize (BaseR base_reglas, BaseD base_datos, MiDataset tabla) {
		...
		/* if there are suficient, the examples are randomly selected */
			...
			for (i=0; i<tercio_pob; i++)
				seleccionados[i] = indices_nc[Randomize.RandintClosed (0,tabla.no_cubiertos-1)];
		}
		...
		for (i=0; i<final_grupo1_2; i++) {
			for (j=0; j<n_variables; j++) {
				...
				for (k=0; k<base_datos.n_etiquetas[j]; k++) {
					grado_pertenencia = base_reglas.Fuzzifica (tabla.datos[seleccionados[i]].ejemplo[j], base_datos.BaseDatos[j][k]);
				...
				for (k=0;k<3;k++)
					New[final_grupo1_2+i].Gene[pos_etiq + k]=Randomize.Randdouble (base_datos.intervalos[j][etiqueta][k].min, base_datos.intervalos[j][etiqueta][k].max);
			}
		...
		for (i = 2 * final_grupo1_2; i<long_poblacion; i++) {
			for (j=0; j<tabla.n_variables; j++) {
				...
				for (k=0; k<3; k++)
					New[i].Gene[pos_etiq + k] = Randomize.Randdouble (base_datos.intervalos[j][etiqueta][k].min, base_datos.intervalos[j][etiqueta][k].max);
			}
	...
	void Cruce_MMA_Simple (int n_variables) {
		...
		for (mom=0; mom<last; mom+=2) {
			...
			while (i<n_variables && misma_regla==1)
				if (New[dad].Gene[i] != New[mom].Gene[i])  misma_regla = 0;
				else i++;
			...
			/* Simple Crossover */
				...
				for (i=xpoint1; i<n_variables; i++) {
					...
					for (j=0; j<3; j++) {
						...
						New[mom].Gene[n_variables+3*i+j] = New[dad].Gene[n_variables+3*i+j];
	...
	public void Mutacion_Thrift_No_Uniforme (long Gen, long n_generaciones, BaseD base_datos) {
		...
		if (prob_mutacion>0) {
			while (Mu_next < posiciones) {
				...
				if (j < base_datos.n_variables) {
					...
					if ((m<0.5 && (New[i].Gene[j] != (double)base_datos.n_etiquetas[j]-1)) || New[i].Gene[j]==0.0)
						nval = New[i].Gene[j]+1.0;     

					else  nval = New[i].Gene[j]-1.0;

					/* we update the membership function */
					variable = j;
					etiqueta = (int) nval;
					New[i].Gene[base_datos.n_variables+3*variable] = base_datos.BaseDatos[variable][etiqueta].x0;
					New[i].Gene[base_datos.n_variables+3*variable+1] = base_datos.BaseDatos[variable][etiqueta].x1;
					New[i].Gene[base_datos.n_variables+3*variable+2] = base_datos.BaseDatos[variable][etiqueta].x3;
				}
				/* else the gene is of C2 */
					...
					/* we mutate the gene */
					if (m<0.5)
						nval = New[i].Gene[j] + delta (Gen, base_datos.intervalos[variable][etiqueta][punto].max-New[i].Gene[j], n_generaciones);
					else
						nval = New[i].Gene[j] - delta (Gen, New[i].Gene[j]-base_datos.intervalos[variable][etiqueta][punto].min, n_generaciones);
				}
	...
	public String SoluciontoString () {
		...
		for (i=0; i<n_variables; i++) {
			...
			cadena += "    " + New[Best_guy].Gene[pos_etiq] + " " + New[Best_guy].Gene[pos_etiq + 1] + " " + New[Best_guy].Gene[pos_etiq + 2] + "\n";
		}
