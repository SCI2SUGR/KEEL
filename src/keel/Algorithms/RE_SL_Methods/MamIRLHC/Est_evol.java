...
class Est_evol {
	...
	public void Estrategia_Evolucion () {
		int i, j, temp, cromosoma, variable, etiqueta, punto, it_sin_exito, n_ya_explotados, n_a_explotar, fin;
		double n, sigma, newval, new_sigma, old_perf;

		/* we order the population by mean of the bubble method */
		for (i=0; i<long_poblacion; i++)  indices[i]=i;
		...
		while ((i < long_poblacion) && (n_ya_explotados < n_a_explotar)) {
			...
			do {
				...
				for (j=base_datos.n_variables; j<n_genes; j++) {
					...
					if (newval <= base_datos.intervalos[variable][etiqueta][punto].min)
						...
						if (newval >= base_datos.intervalos[variable][etiqueta][punto].max)
							Hijo.Gene[j] = base_datos.intervalos[variable][etiqueta][punto].max;
						else
							Hijo.Gene[j] = newval;
					}
				}
			    ...
			    /* we adapt sigma */
				new_sigma = AdaptacionSigma (sigma,(double)n_exitos/(double)n_mutaciones, (double)n_genes-base_datos.n_variables);
