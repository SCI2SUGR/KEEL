...
class Lear_m3 {
	...
	public void leer_conf (){
		...
		alg_gen = new AG (long_poblacion, tabla.n_variables, cruce, mutacion, valor_a, valor_b, fun_adap);
	...
	public void run () {
		...
		do {
			...
			for(i=0; i<tabla.long_tabla; i++) {
				RCE = fun_adap.ReglaCubreEjemplo (alg_gen.solucion(), tabla.datos[i].ejemplo);
		...
		cadenaReglas += "\nMSEtra: " + ec + " MSEMtst: "+ ec_tst + "\nMLEtra: " + el + " MLEtst: "+ el_tst + "\nMinimum of C_R: " + min_CR + " Minimum of CV_R: " + min_CVR + "\n";
		...
		Fichero.AnadirtoFichero(ruta_salida + "MogulHCcomunR.txt", "" + base_reglas.n_reglas + "\n");
		Fichero.AnadirtoFichero(ruta_salida + "MogulHCcomunTRA.txt", "" + ec + "\n");
		Fichero.AnadirtoFichero(ruta_salida + "MogulHCcomunTST.txt", "" + ec_tst + "\n");
	}
	}
