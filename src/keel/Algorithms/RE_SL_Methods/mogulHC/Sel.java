...
class Sel {
  ...
  public void leer_conf() {
    ...
    if (tabla.salir == false) {
      ...
      base_total = new BaseR(fichero_br, tabla);
      base_reglas = new BaseR(base_total.n_reglas, tabla);
      fun_adap = new Adap_Sel(tabla, base_reglas, base_total, porc_num_reglas,
                          n_soluciones, tau, alfa, tipo_fitness);
      alg_gen = new AG_Sel(long_poblacion, base_total.n_reglas, cruce, mutacion,
                       fun_adap);
    }
  }
    ...
    if (tabla.salir == false) {
      ...
      do {
        ...
        if (Aceptar(alg_gen.solucion()) == 1) {
          ...
          Fichero.AnadirtoFichero(ruta_salida + "selcomunR.txt",
                                  "" + base_reglas.n_reglas + "\n");
          Fichero.AnadirtoFichero(ruta_salida + "selcomunTRA.txt",
                                  "" + ec + "\n");
          Fichero.AnadirtoFichero(ruta_salida + "selcomunTST.txt",
                                  "" + ectst + "\n");
        }
