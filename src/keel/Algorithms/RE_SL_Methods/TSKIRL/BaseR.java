  ...
  ------------------------------------------------------------------------- */
	...
	void CompruebaEjemplos (int [] Antecedente, Adap fun_adap) {
		...
		while (i<tabla.long_tabla && hay_ejemplos==0) {
			if (fun_adap.AntecedenteCubreEjemplo (cromosoma, tabla.datos[i].ejemplo)>0)
				hay_ejemplos=1;
			else  i++;
		}
	...
	void RecorreAntecedentes (int [] Regla_act, int pos, Adap fun_adap) {
		...
		if (pos==tabla.n_var_estado)
			...
			for (Regla_act[pos]=0; Regla_act[pos]<base_datos.n_etiquetas[pos]; Regla_act[pos]++)
	...
	public void inserta_regla (int regla, double [] datos) {
		...
		for (i=0; i<tabla.n_var_estado; i++) {
			...
			BaseReglas[regla].Cons[i] = Math.tan (datos[tabla.n_var_estado + tabla.n_var_estado * 3 + i]);
     	}

		/* 'b' value of the consequent */
		BaseReglas[regla].Cons[i] = Math.tan (datos[tabla.n_var_estado + tabla.n_var_estado*4]);
	}
