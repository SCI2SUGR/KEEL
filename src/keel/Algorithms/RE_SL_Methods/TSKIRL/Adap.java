...
class Adap {
	...
	public Adap (MiDataset training, BaseR base_r, double valor_omega, double valor_k, double valor_epsilon, int t_fitness, int t_nichos) {
		int i;
...
/* -------------------------- Medida Local de Error ----------------------- */
	...
	public void CriteriosReglas (double [] cromosoma) {
		...
		for (i=0; i<tabla.long_tabla; i++) {
			...
			if (RCE== 0.0 && EmparejaAnt>0)  /* Si estan cubiertas las entradas pero no las */
		...
		if (n_ejem_pos > 0)  G = SumaRCEpositivos/(double)n_ejem_pos;
		else  G =  0.0;
		...
		if (n_ejem_neg <= umbral)  g = 1.0;
		else  g = 1.0 / (n_ejem_neg - umbral + Math.exp(1.0));
		...
		if (tipo_fitness==2) {
			...
			for (i=0; i<tabla.long_tabla; i++) {
				...
				if (tabla.datos[i].cubierto>0 && RCE!=0)  n_ejem_ya_cub++;

				/* Se efectua la penalizacion correpondiente */
				if (n_ejem_ya_cub <= umbral)  PC = 1.0;
				else  PC = 1.0 / (n_ejem_ya_cub - umbral + Math.exp(1.0));
			}
		}
	}
...
/* ---------------------------- Funcion fitness --------------------------- */
	...
	public void ejemplos_positivos (double [] ant) {
		...
		for (i=0; i < tabla.long_tabla; i++) {
			tabla.datos[i].nivel_cubrimiento = AntecedenteCubreEjemplo (ant, tabla.datos[i].ejemplo);
...
/* ---------------------------- Funcion fitness --------------------------- */
	...
	public double eval_11 (double [] cromosoma, int aplicar_ee_11) {
		if (tipo_fitness == 1)  return (eval_mulmodal(cromosoma, aplicar_ee_11));
		else  return (eval_criterios (cromosoma, aplicar_ee_11));
	}
	...
	public double eval_mulmodal (double [] cromosoma, int aplicar_ee_11) {
		...
		fitness = 1;
		if ((F==0) || (G==0))  fitness = Double.MAX_VALUE;
		else  fitness /= (F*G);

		/* Si se esta aplicando la EE, consideramos los nichos. Si no es asi, no */
		if (aplicar_ee_11==1) {
			nicho = LNIR (cromosoma);
			if (nicho > 0) fitness /= nicho;
			else fitness = Double.MAX_VALUE;
		}
   ...
   ------------------------------------------------------------------------- */
	...
	public double NIR1 (double [] cromosoma) {
		...
		for (i=0; i < base_reglas.n_reglas; i++) {
			...
			if (cubr_act>max_cubr)  max_cubr=cubr_act;
		}
	...
	public double NIR2 (double [] cromosoma) {
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			/* Punto izquierdo */
			for (j=0; j<tabla.n_var_estado; j++)  puntos[j] = base_reglas.BaseReglas[i].Ant[j].x0;

			cubr_act[0] = AntecedenteCubreEjemplo (cromosoma, puntos);
			if (cubr_act[0] > max_cubr)   max_cubr = cubr_act[0];

			/* Centro */
			for (j=0; j<tabla.n_var_estado; j++)   puntos[j] = base_reglas.BaseReglas[i].Ant[j].x1;

			cubr_act[1] = AntecedenteCubreEjemplo (cromosoma, puntos);
			if (cubr_act[1] > max_cubr)  max_cubr = cubr_act[1];

			/* Punto derecho */
			for (j=0; j < tabla.n_var_estado; j++)  puntos[j] = base_reglas.BaseReglas[i].Ant[j].x3;
			cubr_act[2] = AntecedenteCubreEjemplo (cromosoma, puntos);
			if (cubr_act[2] > max_cubr)  max_cubr = cubr_act[2];
		}
	...
	public double NIR3 (double [] cromosoma) {
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			cubr_act = AntecedenteCubreEjemplo (cromosoma, base_reglas.ListaTabu[i]);
			if (cubr_act > max_cubr_otras)  max_cubr_otras = cubr_act;
		}
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			cubr_act = 1.0;
			for (j=0; j<tabla.n_var_estado; j++) {
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)+1], base_reglas.BaseReglas[i].Ant[j]);
				if (grado_pertenencia<cubr_act)  cubr_act = grado_pertenencia;
			}

			if (cubr_act > max_cubr_nueva)  max_cubr_nueva = cubr_act;
		}
	...
	public double NIR4 (double [] cromosoma) {
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			...
			cubr_act[0] = AntecedenteCubreEjemplo (cromosoma, puntos);
			if (cubr_act[0] > max_cubr_nueva)   max_cubr_nueva=cubr_act[0];

			/* Centro */
			for (j=0; j<tabla.n_var_estado; j++)  puntos[j]=base_reglas.BaseReglas[i].Ant[j].x1;

			cubr_act[1] = AntecedenteCubreEjemplo (cromosoma, puntos);
			if (cubr_act[1] > max_cubr_nueva)  max_cubr_nueva = cubr_act[1];

			/* Punto derecho */
			for (j=0; j<tabla.n_var_estado; j++)  puntos[j] = base_reglas.BaseReglas[i].Ant[j].x3;

			cubr_act[2] = AntecedenteCubreEjemplo (cromosoma, puntos);
			if (cubr_act[2] > max_cubr_nueva)  max_cubr_nueva = cubr_act[2];
		}
		...
		for (i=0; i<base_reglas.n_reglas; i++) {
			...
			for (j=0; j<tabla.n_var_estado; j++) {
				/* Punto izquierdo */
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)], base_reglas.BaseReglas[i].Ant[j]);
				if (grado_pertenencia < cubr_act[0])  cubr_act[0] = grado_pertenencia;

				/* Centro */
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)+1], base_reglas.BaseReglas[i].Ant[j]);
				if (grado_pertenencia < cubr_act[1])  cubr_act[1] = grado_pertenencia;

				/* Punto derecho */
				grado_pertenencia = base_reglas.Fuzzifica (cromosoma[tabla.n_variables+(3*j)+2], base_reglas.BaseReglas[i].Ant[j]);
				if (grado_pertenencia < cubr_act[2])  cubr_act[2] = grado_pertenencia;
			}

			if (cubr_act[0] > max_cubr_otras)  max_cubr_otras = cubr_act[0];
			if (cubr_act[1] > max_cubr_otras)  max_cubr_otras = cubr_act[1];
			if (cubr_act[2] > max_cubr_otras)  max_cubr_otras = cubr_act[2];
		}
...
/* -------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
	...
	public double LNIR (double [] cromosoma) {
		...
		switch (tipo_nichos) {
			case 1: salida = 1 - NIR1 (cromosoma); break;
			case 2: salida = 1 - NIR2 (cromosoma); break;
			case 3: salida = 1 - NIR3 (cromosoma); break;
			case 4: salida = 1 - NIR4 (cromosoma); break;
		}
