...
class Est_mu_landa {
	...
	public Est_mu_landa (BaseR base_r, Adap fun, MiDataset t, int gen_ee, int m, int l, int n_sigm, int n_alf, int Omeg_x, int Omeg_sigma, int Omeg_alfa, int Delt_x, int Delt_sigma, int Delt_alfa) {
		int i;
...
/* -------------------------------------------------------------------------
  ...
  ----------------------------------------------- -------------------------- */
	...
	public void InicializaPadres () {
		...
		if (fun_adap.n_ejemplos_positivos>0)  y_med /= fun_adap.n_ejemplos_positivos;
		else  y_med = Double.MAX_VALUE;
...
/* -------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------- */
	...
	public void inicializa_cons (Structure Padre) {
		...
		for (i=0; i<tabla.n_var_estado; i++)  Padre.Gene[pos_individuo+i] = 0;

		Padre.Gene[pos_individuo+tabla.n_var_estado] = tabla.datos[indices_ep[0]].ejemplo[tabla.n_var_estado];
	}
