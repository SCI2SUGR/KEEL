	...
	public void Estrategia_Evolucion (Structure Padre, Structure Hijo) {
		...
		do {
			...
			if (Hijo.Perf < Padre.Perf) {
				...
				for (j=0; j<n_genes_ee_11; j++)  Padre.Gene[j] = Hijo.Gene[j];
			...
			/* we adapt sigma */
			new_sigma = AdaptacionSigma (sigma, n_exitos / (double)n_mutaciones, (double)n_genes_ee_11-n_var_estado);

			if (it_sin_exito >= n_gen_ee_11)  fin = 1;
