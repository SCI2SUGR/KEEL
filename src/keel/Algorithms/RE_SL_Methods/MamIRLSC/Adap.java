...
class Adap {
    ...
    public void Error_tra() {
        ...
        for (j = 0, suma1 = suma2 = 0.0; j < tabla.long_tabla; j++) {
            ...
            suma1 += 0.5 *
                    Math.pow(tabla.datos[j].ejemplo[tabla.n_var_estado] - fuerza,
                             2.0);
            suma2 +=
                    Math.abs(tabla.datos[j].ejemplo[tabla.n_var_estado] - fuerza);
        }
    ...
    public double eval(double[] cromosoma, int aplicar_ee) {
        if (tipo_fitness == 1) {
            return (eval_mulmodal(cromosoma, aplicar_ee));
        } else {
            return (eval_criterios(cromosoma, aplicar_ee));
        }
    }
