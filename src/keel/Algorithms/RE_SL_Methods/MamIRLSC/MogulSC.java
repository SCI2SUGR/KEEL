...
class MogulSC {
    ...
    public void leer_conf() {
        ...
        MaxReglas = (new Double(Math.pow(n_etiquetas, tabla.n_var_estado))).
                    intValue();
    ...
    public void run() {
        ...
        if (tabla.salir == false) {
            ...
            do {
                ...
                System.out.println("Number of rules generated: " +
                                   (base_reglas.n_reglas + 1));
                ...
                for (i = 0; i < tabla.long_tabla; i++) {
                    RCE = fun_adap.ReglaCubreEjemplo(Padre.Gene,
                            tabla.datos[i].ejemplo);
                    tabla.datos[i].nivel_cubrimiento += RCE;
                    tabla.datos[i].maximo_cubrimiento = Adap.Maximo(tabla.datos[
                            i].maximo_cubrimiento, RCE);
            ...
            Fichero.AnadirtoFichero(ruta_salida + "MogulSCcomunR.txt",
                                    "" + base_reglas.n_reglas + "\n");
            Fichero.AnadirtoFichero(ruta_salida + "MogulSCcomunTRA.txt",
                                    "" + ec + "\n");
            Fichero.AnadirtoFichero(ruta_salida + "MogulSCcomunTST.txt",
                                    "" + ec_tst + "\n");
        }
    }
    ...
    public void Generate() {
        ...
        for (i = 0; i < tabla.no_cubiertos; i++) {
            ...
            for (j = 0; j < tabla.n_variables; j++) {
                ...
                for (k = 0; k < base_datos.n_etiquetas[j]; k++) {
                    grado_pertenencia = base_reglas.Fuzzifica(tabla.datos[
                            indices_nc[i]].ejemplo[j],
                            base_datos.BaseDatos[j][k]);
            ...
            if (Pertenece(n_reg_generadas) == 0) {
                for (j = 0; j < tabla.n_variables; j++) {
                    ...
                    base_reglas.Pob_reglas[n_reg_generadas].Gene[j] = (double)
                            etiqueta;
        ...
        for (i = 0; i < n_reg_generadas; i++) {
            base_reglas.Pob_reglas[i].Perf = fun_adap.eval(base_reglas.
                    Pob_reglas[i].Gene, aplicar_ee);
