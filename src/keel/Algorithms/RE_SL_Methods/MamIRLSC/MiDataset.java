...
public class MiDataset {
    ...
    public void processModelDataset(String nfejemplos, boolean train) throws
        ...
        try {
            ...
            if (Attributes.hasNominalAttributes()) {
                ...
                calculaRangos(); //recalcula "extremos" a partir de la cabecera
                long_tabla = k;
            }
        } catch (Exception e) {
            System.out.println("DBG: Exception in readSet");
    ...
    /**
     * Calcula el universo de discuros de las variables de entrada y salida
     */
    public void calculaRangos() {
        for (int i = 0; i < this.n_var_control; i++) {
            if (Attributes.getInputAttribute(i).getNumNominalValues() > 0) {
                extremos[i].min = 0;
                extremos[i].max = Attributes.getInputAttribute(i).
                                  getNumNominalValues() - 1;
            } else {
                extremos[i].min = Attributes.getInputAttribute(i).
                                  getMinAttribute();
                extremos[i].max = Attributes.getInputAttribute(i).
                                  getMaxAttribute();
            }
        }
        extremos[n_variables -
                1].max = Attributes.getOutputAttribute(0).getMinAttribute();
        extremos[n_variables -
                1].max = Attributes.getOutputAttribute(0).getMaxAttribute();
    }
