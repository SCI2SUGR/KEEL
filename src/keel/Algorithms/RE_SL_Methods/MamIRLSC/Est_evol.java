    ...
    public void Estrategia_Evolucion(Structure Padre, Structure Hijo) {
        ...
        do {
            for (variable = 0; variable < base_datos.n_variables; variable++) {
                ...
                S = Adap.Minimo(x0 -
                                base_datos.intervalos[variable][etiqueta].min,
                                newx1 - x0) / 2.0;
                n = ValorNormal(new_sigma * S);
                newx = x0 + n;
                ...
                S = Adap.Minimo(x2 - newx1,
                                base_datos.intervalos[variable][etiqueta].max -
                                x2) / 2.0;
                n = ValorNormal(new_sigma * S);
                newx = x2 + n;
                ...
                if (newx <= newx1) {
                    ...
                    if (newx >= base_datos.intervalos[variable][etiqueta].max) {
                        Hijo.Gene[gen +
                                2] = base_datos.intervalos[variable][etiqueta].
                                     max;
                    } else {
            ...
            /* we evaluate the son */
            Hijo.Perf = fun_adap.eval(Hijo.Gene, 1);
