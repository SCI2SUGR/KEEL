...
public class FRNN_VQRS extends FuzzyIBLAlgorithm {
	 ...
	 * @param script Configuration script
	 */
	public FRNN_VQRS(String script){
		
		readDataFiles(script);
		
		//Naming the algorithm
		name="Vaguely quantified nearest neighbor";
	...
	private int classifyInstance(int index, boolean train){
		...
		for (int l = 0; l< K; l++){	
			quality=VQ1(R[l]/sum);
			lower[trainOutput[nearestN[l]]]=Math.max(lower[trainOutput[nearestN[l]]],quality);
			quality=VQ2(R[l]/sum);
			upper[trainOutput[nearestN[l]]]=Math.max(upper[trainOutput[nearestN[l]]],quality);
		}
	...
	private double VQ1(double x){
		...
		if(x<=(alpha+beta)/2.0){
			return (2.0*((x-alpha)*(x-alpha)))/((beta-alpha)*(beta-alpha));
		}
		
		if(x<=beta){
			return 1.0-(2.0*((x-alpha)*(x-alpha)))/((beta-alpha)*(beta-alpha));
		}
