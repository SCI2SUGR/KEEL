...
public class Regla {
    ...
    //la consistencia se genera en la funicon getScore()
   ...
   /**
     ...
     * @param instances Instances Objeto que contiene todas la filas del dataset de entrenamiento
     * @return Double Valor de la m-probability-estimate de la regla
     */
    public Double getScore(Instances instances){
        
        Integer casos_positivos=0;
        Integer casos_negativos=0;
        Integer k = instances.getnClass();
        ...
        if (!this.getAntecedente().isEmpty()){
            for(int i=0;i<instances.getnData();i++){ //Para cada fila
                ...
                LinkedList<Double> fila = instances.getExample(i); //cogemos la fila 
                String valor_salida = instances.getOutputAsString(i);//cogemos el valor de salida de la fila
            ...
            for(int i=0;i<instances.getnData();i++){ //Para cada fila
                String valor_salida = instances.getOutputAsString(i);
                if (this.getConsecuente().equals(valor_salida))
                    num_mismo_valor_salida++; //almacena P, que se utiliza en la P0 = P/N
            }  
        } 
        //se aprovecha el calculo para calcular la consistencia de la regla
        if(casos_negativos == 0) this.consistencia=true;
        
        //Po = P/N
        double p = (num_mismo_valor_salida.doubleValue()/instances.getnData());
        
        //m-probability-estimate (m = n_positivos + k * Po/n_pos_neg + k
