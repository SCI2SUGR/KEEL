...
public class Itemset 
{
    ...
    public void setValue( int index, double value )
	{
        ...
        // Modified to allow the grow of the array of attributes
        double[] help = new double[index >= values.length ? index + 1 : values.length];
     ...
     * 
     * @param index given index attribute.
    ...
    public String toString()
	{
    	...
    	for ( int i = 0; i < dataset.numAttributes(); i++ )
    	{
    		...
    		if ( att.isContinuous() )
    			result += att.name() + "=" + (isMissingValue(values[i]) ? "*" : values[i]) + "\n";
    		else
    			result += att.name() + "=" + (isMissingValue(values[i]) ? "*" : att.value( (int)values[i] )) + "\n";
    	}
