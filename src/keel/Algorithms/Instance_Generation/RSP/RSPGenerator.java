...
public PrototypeSet reduceSet()
  {
	  ...
	  /*
	   * RSP3=
	  En esta aproximaci�n, el par�metro inicial 'b' no es necesario. Siempre y cuando dentro
	  de un bloque tengamos mezcla entre dos o m�s clases, ese bloque se va a dividir. Por
	  tanto, no se conoce apriori el n�mero de bloques final.

	  Por ejemplo, suponte que tienes un bloque con 5 ejemplos de la clase A y 4 de la B.
	  Divides, calculas los dos m�s alejados y consigues un subbloque con 3 ejemplos de la
	  clase A y los 4 de la B, mientras que el otro tiene el resto (2 de la clase A). El
	  primer bloque se tiene que dividir de nuevo (cuando le toque, depender� del solapamiento
	  y/o di�metro comparado con los dem�s). El segundo ya es homog�neo.
	  */
    ...
    //	  C.add(new PrototypeSet());    	  
     // }
	  ...
	  for ( bc=1; (bc <this.numberOfBlocks || (rsp3 && !everyHomogenity)); bc++){ //
         ...
         /* Esto solo modifica el punto 9, la selecci�n del bloque m�s apropiado entre los que
          tienes disponibles.
          Elegir entre ovelapping o distancia m�s lejanos.
      ...
      for(i=0; i<bc; ++i)
      {
    	  for( int j= 0; j< numberOfClass; j++){
    		  ...
    		  if(aux.size()>0){ // Checking there is this class in the subset.
	    		  ...
	    		  result.add(averaged.formatear()); // FORMATEANDO AQU�
    		  }
