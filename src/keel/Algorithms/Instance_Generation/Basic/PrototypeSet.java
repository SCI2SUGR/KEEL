...
public class PrototypeSet extends ArrayList<Prototype> implements Comparable
{
    ...
    */
		...
		while (cambio) {
			...
			for (i=0; i<datosTrain.length; i++) {
				...
				if (clusters[i] != pos) {
					...
					//System.out.println("Estas aki no mamï¿½n");
    ...
    /**
     * SUMAR dos conjuntos de prototipos , uno a uno. De la misma dimensiï¿½n.
     */
   ...
   public PrototypeSet sumar(PrototypeSet other){
     ...
     /**
      * Restar dos conjuntos de prototipos , uno a uno. De la misma dimensiï¿½n.
      */
   ...
   public String [][] toBinaryString (){
	...
	for(int i=0; i< this.size();i++){
		...
		for(int j=0; j<((Prototype)this.get(i)).numberOfInputs(); j++ ){
			...
			num = (int) (number/parametroConversion); // Nos quedamos con la parte entera de la conversiï¿½n
			
			aux = Integer.toBinaryString(num);
			//Tendremos que aï¿½adir ceros al a derecha...si es menor que 0.
			while(aux.length()<8){
				aux = "0"+aux;
			}
   ...
   public void toPrototypeSet (String datos[][], double clases[]){
	   ...
	   for(int i=0; i< datos.length;i++){
		   ...
		   for(int j=0; j< datos[i].length; j++){
			   ...
			   // Transfomraciï¿½n Gray to binary..
			   aux2 += datos[i][j].charAt(0); // El mï¿½s a la izquierda es igual
			   char uno, dos;
			   
			   for(int k=1; k< 8;k++){
				   uno = aux2.charAt(k-1); // El primero de Binary con el 2ï¿½ de Gray
