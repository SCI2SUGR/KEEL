...
public class AMPSOGenerator extends PrototypeGenerator {
  ...
  protected double fitness(PrototypeSet Population, int particleIndex){
	  ...
	  PrototypeSet b = new PrototypeSet();
 		...
 		/*	suponte que en {g} tenemos tres patrones. D_{g,i} es la distancia de la part�cula al patr�n i-�simo. 
		La expresi�n G_f hace la sumatoria de todas ellas a la inversa. No tiene dificultad alguna.
		Es un fitness local que se calcula por part�cula. 
		Una part�cula clasificar� correcta e incorrectamente una seria de patrones 
		(aquellos que la tienen de vecina m�s cercana de la misma clase, los primeros; y de distinta clase, los segunndos). 
		Esos son el conjunto {b} y el {g}. Se calcula la distancia a cada uno de los ptarines para obtener la 
		sumatorias de la derecha de la expresi�n (3) y ya calcula el total fitness dependiendo si {b} o {g} est�n vac�os o no.
		 */
	  
	  // Respecto del Training Set. Conjunto de Patrones
	  for(int i = 0; i< trainingDataSet.size(); i++){
			  Prototype nearest = Population.nearestTo(trainingDataSet.get(i));
		    // Compruebo si el prototipo m�s cercano es particleIndex
	 ...
	 // Velocities.print();
	  ...
	  RepulsionCenter = Population.clone();
	   ...
	   for(int iter=0; iter< MaxIter && success_rate != 100.0; iter++){ 
		    ...
		    * Yo, por ejemplo, tendr�a un vector de booleanos para cada conjunto que indica si pertenece o no al conjunto competitivo o no 
		    * competitivo. Para ver si pertenecen, te ci�es a lo que pone en la secci�n 3.4: "Si un prototipo clasifica mal a otro,
		    * se incluye en el set no-competing". Esto se traduce a que si el prototipo m�s cercano a
		    *  uno dado es de otra clase, �ste se incluye en el no-competing (pones su bit a true). IDEM para el set competing.
		    */
	...
	*/	   
		   ...
		   for(int i=0; i< this.numberOfPrototypes; i++){
			   ...
			   //Find the closet particle in the non-competing set for particle class.
	   			/*
	   			 * Justo despu�s, en el punto 4(c) iii; que como ves se hace para cada part�cula, se localiza el protitpo m�s cercano
	   			 *  a esta part�cula que pertenezca al conjunto de no competitivos. Esto es, coges tu vector de booleanos no-competing 
	   			 *  y calculas el prototipo m�s cercano de aquellos cuyo bit est� a true. �ste es tu centro de atracci�n
	   			 *  (l�gico, la part�cula se mueve hacia un lugar donde pueda evitar una mal clasificaci�n, y �ste ser� el m�s cercano).
