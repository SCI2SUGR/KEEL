...
public class missing {
	...
	static fuzzy[][] values_missing(fuzzy X[][],int nejemplos,int dimx, int m)
	{
		...
		for(int i=0;i<dimx;i++)
		{
			...
			{
				if(X[j][i].getizd()!=-2 && X[j][i].getdere()!=-2)
				{
					mean_iz=mean_iz+X[j][i].getizd();
					mean_der=mean_der+X[j][i].getdere();
					contador++;
					
					frequent_iz.add(X[j][i].getizd());
					frequent_der.add(X[j][i].getdere());
					
				}
				
				if(X[j][i].getcent()!=-2)
				{
					mean_centro=mean_centro+X[j][i].getcent();
					frequent_centro.add(X[j][i].getcent());
				}
			}
