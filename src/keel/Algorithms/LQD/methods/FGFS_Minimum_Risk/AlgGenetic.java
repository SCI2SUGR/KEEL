...
public class AlgGenetic {
  ...
  Vector<Vector<Integer>> confusion_matrix;
    ...
    public AlgGenetic(int npoblation,  float muta,float cross,Vector<fuzzyPartition> parte, 
    		int nclases,fuzzy[][] x,Vector<Vector<Float>> y, Vector<Float> cost_example,int niteration,
    ...
    public void evaluate_poblation() throws IOException
    {
    	...
    	for (int i=0;i<X.length;i++)//x.length indicates the number of example in the training 
		{
    			...
    			if(exist==false)// is a winner rule
    			{
    				if(COST==1)//with cost matrix, but the fitness train is zero-one loss 
    				{
    					...
    					for(int p=0;p<point.size();p++)
    					{
    						...
    						if(points.contains((point.get(p)*P.get(i)))==false)
    						{
    							points.addElement(point.get(p)*P.get(i));
    					...
    					for(int p=0;p<point.size();p++)
    					{
    						if(points_cost.contains(point.get(p)*P.get(i))==false)
    							points_cost.addElement(point.get(p)*P.get(i));
    					}
    		...
    		if((winner.size()!=0 && distribute==1) ||( win!=-1 && distribute==0))
    		{
    			if(COST==0)
    			{
    				if(distribute==1)
    				{
    					...
    					{
    						fitness.set(winner.get(g),summation(fitness.get(winner.get(g)),reparto_puntos)); 
    		...
    		calculation_fitness_total(points);//The total fitness is expressed by zerp-one loss (also if the training
   ...
   public Vector<Float> summation(Vector<Float> values_r,Vector<Float> punto) throws IOException
    ...
    public Vector<Float> puntuation(int rule, Vector<Float> output, int test) throws IOException
    {
		...
		if(rule!=-1)
    	{
    		Float [] cons=poblation.get(rule).getregla().getConsecuente();
    ...
    public void show_fitness_total(int last) throws IOException
    {
    	...
    	for (int i=0;i<X.length;i++)    	
    		peso=peso+P.get(i); //obtain the number of instances (throught the duplication in cost)
    	
    	for (int i=0;i<fitness_total.size();i++)
    	{
    		if(minimo>(1-fitness_total.get(i)/peso))
    			minimo= (1-fitness_total.get(i)/peso);
    		
    		if(maximo<(1-fitness_total.get(i)/peso))
    		{
    			maximo=(1-fitness_total.get(i)/peso);
    		}
    	}
	...
	void test() throws IOException
	{
        ...
        int nexamples;//number of instances    


        String number= "";
        int dataset=1000;
        if(fichero_test.contains("C_LQD")==true)
        	dataset=1;
        for(int c=0;c<dataset;c++)
        {
        	...
        	if(fichero_test.contains("C_LQD")==false)
        		contador=Integer.parseInt(input.readLine());
        	dimx=Integer.parseInt(input.readLine());        
            ncol= dimx+1;        
            nexamples=Integer.parseInt(input.readLine());        
            float X[][] = new float[nexamples][dimx];
            Vector<Vector<Float>> C= new Vector<Vector<Float>>(nexamples);//la salida sera un conjunto de elementos
            ...
            for(int i=0; i<nexamples; i++)
            {
            	for(int j=0; j<ncol-1; j++)
            	{
        			...
        			if(caracter==' ' )
        			{
        				...
        				{
        						X[i][j]=Float.parseFloat(number);
        				}
        				number="";
        			}
            	}
        		
            	//	Read the output of the instances {1,..,x}
            	caracter = (char)input.read();
            	Vector <Float> output_imp= new Vector<Float>();
            	while(caracter!='}')
            	{
            		caracter = (char)input.read();
            		while(caracter!=',' && caracter!='}')
            		...
            		output_imp.addElement(Float.parseFloat(number));
            		number="";
            ...
            Vector<Integer> eliminate= new Vector<Integer>();
            eliminate=Missing.values_missing_test(X, nexamples,dimx);
        
            float X1[][] = new float[(nexamples-eliminate.size())][dimx];
            Vector<Vector<Float>> C1= new Vector<Vector<Float>>((nexamples-eliminate.size()));//output is a set of elements
            int pos=0;
            for(int x1=0; x1<nexamples;x1++)
            {
		 ...
		 else
		 {
			...
			if(COST==0)
			{
				...
				for (int i=0;i<confusion_matrix.size();i++) 
		    	{
		    		...
		    		for (int j=0;j<confusion_matrix.size();j++) 
		        	{
		    			//fs1.write("["+i+","+j+"]= "+((float)confusion_matrix.get(i).get(j))+" ");
	 ...
	 public void create_file_columns(int regla, Vector<Float> salida) throws IOException
	 {
		 ...
		 if(regla!=-1)
		 {
		  Float [] cons=poblation.get(regla).getregla().getConsecuente();
