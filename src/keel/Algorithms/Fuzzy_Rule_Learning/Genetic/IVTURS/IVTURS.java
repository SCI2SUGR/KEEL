...
public class IVTURS {
	...
	String outputTr, outputTst, fileDB, fileRB, data, evolution;
	String fileTime, fileHora, fileRules, fileEvo;
	...
	int nLabels, populationSize, depth, K, maxTrials, typeInference, BITS_GEN, ajuste, sec;
	 ...
	 */
	public IVTURS(parseParameters parameters) {
		...
		/*
		this.fileTime = parameters.getOutputFile(1) + "_time.txt";
		this.fileHora = parameters.getOutputFile(1) + "_hora.txt";
		this.fileRules = parameters.getOutputFile(1) + "_rules.txt";
		this.fileEvo = parameters.getOutputFile(1) + "_evolution.txt";
		*/
	...
	public void execute() {
		...
		if (this.somethingWrong) { //We do not execute the program
			...
			this.ruleBase = new RuleBase(this.dataBase, this.train, this.test, this.K, this.typeInference);

			this.apriori = new Apriori(this.ruleBase, this.dataBase, this.train, this.test, this.minsup, this.minconf, this.depth);
			...
			System.out.println("Accuracy in train: " + this.ruleBase.getAccuracy());
			this.ajuste = 1;
			this.sec = 0;
			if (this.ajuste != 0){
				//EVOLUTIONARY PROCESS
				this.maxTrials = 20000;
				pop = new Population(this.train, this.dataBase, this.ruleBase, this.populationSize, this.BITS_GEN, this.maxTrials, this.alpha, this.ajuste);
				pop.Generation();
			...
			*/
			this.ruleBase.evaluate();
			this.ruleBase.evaluateTest();
			System.out.println("Classification accuracy in train: " + this.ruleBase.getAccuracy());
			System.out.println("Classification accuracy in test: " + this.ruleBase.getAccuracyTest());
	...
	private String classificationOutput(double[] example) {
		String output = new String("?");
		/*Here we should include the algorithm directives to generate the classification output from the input example */
