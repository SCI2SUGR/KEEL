...
public class myDataset {
  ...
  private double classOverlapping[];
  private int nExamplesClass[], outliers[];
  ...
  /**
   * It computes the class-overlapping rate. It depends on the distribution of distinct classes in training data
   */
  public void computeOverlapping() {
    ...
    for (i = 0; i < nClasses; i++) {
      if (nExamplesClass[i] > 0) {
        classOverlapping[i] = (1.0 - (outliers[i] * 1.0 / nExamplesClass[i]));
      }
      else {
  ...
  public boolean[] getNominals() {
    ...
    for (int i = 0; i < nInputs; i++) {
      nominals[i] = (this.getType(i) == this.NOMINAL);
    }
    return nominals;
  }
