...
public class GeneticFuzzyAprioriDCProcess {
  ...
  private ArrayList<FuzzyAttribute> runGeneticAlgorithm() {
	  ...
	  if (! this.idOfAttributes.isEmpty()) {
		  ...
		  setPop = this.initializePopulation();
  ...
  private ArrayList<ArrayList<Chromosome> > initializePopulation() {
	  ...
	  Gene[] genes;
	  Chromosome chr;
	  ArrayList<ArrayList<Chromosome> > setPopInit;
  ...
  */
  private void mutate(ArrayList<Chromosome> pop) {
	  ...
	  for (p=0; p < pop.size(); p++) {
		  if (Randomize.Rand() < this.pm) {
			  ...
			  genes = chr.getGenes();
			  id_attr = 0;//id_attr = Randomize.Randint(0, genes.length);
  ...
  private void evaluateFitness(Chromosome c) {
	  ...
	  fitness = sumFuzzySupport / suitability;
