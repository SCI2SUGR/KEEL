...
public class Gene {
	...
	// distance gen: cuando los intervalos son positivos o negativos los trato igual, basado en dist Solap y cuando son uno negativo y otro positivo genero los positivos a partir del negativo y comparo las distancias
	public double distanceGen(Gene g){

		double lb_posit, ub_posit, distance, distanceOverlap;
		Gene genP1, genP2, gP1,gP2;
		ArrayList<Gene> listGen, listG;
		int countGenes;

		listGen = new ArrayList();
		listG = new ArrayList<>();

		if(((this.pn) && (g.pn)) || ((!this.pn) && (!g.pn))){
			genP1 = new Gene();
			genP1.setLowerBound(this.lb);
			genP1.setUpperBound(this.ub);
			return 1- relativeDistanceOverlap(g, genP1)/2;
			...
			distanceOverlap = 0;
			gP1 = new Gene();
			gP1.setLowerBound(g.lb);
			gP1.setUpperBound(g.ub);
			gP2 = null;
			...
			if(!g.pn){ // g negative interval
				if(g.lb == this.min_attr){
					...
					gP1.setLowerBound(lb_posit);
					gP1.setUpperBound(ub_posit);
					// return 1- distanceOverlap(gen)/(Math.max(this.ub,g.ub) - Math.min(this.lb,g.lb));
				}
				if(g.ub == this.max_attr){
					lb_posit = this.min_attr;
					ub_posit = g.lb;
				...
				if((g.lb != this.min_attr) && (g.ub != this.max_attr)){
					gP1.setLowerBound(this.min_attr);
					if(type == myDataset.REAL) gP1.setUpperBound(g.lb - 0.0001);
					else  gP1.setUpperBound(g.lb - 1);

					gP2 = new Gene();
					if (type == myDataset.REAL)  gP2.setLowerBound(g.ub + 0.0001);
					else  gP2.setLowerBound(g.ub + 1);

					gP2.setUpperBound(this.max_attr);
				}
			} 

			if(!this.pn){ // this negative interval
				if(this.lb == this.min_attr){
					lb_posit = this.ub;
					ub_posit = this.max_attr;
					if (type == myDataset.REAL)  lb_posit+= 0.0001;
					...
					genP1.setLowerBound(lb_posit);
					genP1.setUpperBound(ub_posit);
				}		
				if(this.ub == this.max_attr){
					lb_posit = this.min_attr;
					ub_posit = this.lb;
				...
				if((this.lb != this.min_attr) && (this.ub != this.max_attr)){
					genP1.setLowerBound(this.min_attr);
					if(type == myDataset.REAL) genP1.setUpperBound(this.lb - 0.0001);
					else  genP1.setUpperBound(this.lb - 1);

					genP2 = new Gene();
					if (type == myDataset.REAL)  genP2.setLowerBound(this.ub + 0.0001);
					else  genP2.setLowerBound(this.ub + 1);

					genP2.setUpperBound(this.max_attr);
				}

			}

			if(genP1!=null) listGen.add(genP1);
			if(genP2!=null) listGen.add(genP2);
			if(gP1!=null) listG.add(gP1);
			if(gP2!=null) listG.add(gP2);

			for(int i=0; i< listG.size(); i++){
				for(int j=0; j< listGen.size(); j++){
					distance+= relativeDistanceOverlap(listGen.get(j),listG.get(i));
				}
			}
			return 1 - distance/(listG.size() + listGen.size());

		}
	}
	public double relativeDistanceOverlap(Gene g, Gene gen){

		double relativeDist;

		if((gen.ub == gen.lb) || (g.ub == g.lb)){ // cuando no representan un intervalo sino son un punto porque lb == ub
			if(Math.max(gen.lb,g.lb) > Math.min(gen.ub,g.ub)) relativeDist=0; // no hay solapamiento entre los puntos
			else relativeDist = 1; // hay solapamiento: los puntos son iguales o el intervalo cubre al punto
		}
		else{
			if(Math.max(gen.lb,g.lb) > Math.min(gen.ub,g.ub)) relativeDist = 0;

			else relativeDist= (Math.min(gen.ub,g.ub) - Math.max(gen.lb,g.lb))/(gen.ub - gen.lb) + (Math.min(gen.ub,g.ub) - Math.max(gen.lb,g.lb))/(g.ub - g.lb);
		}
	...
	public double distanceGenMaxOverlap(Gene g){
		Gene genP1;
		...
		if(((this.pn) && (g.pn)) || ((!this.pn) && (!g.pn))){
			...
			genP1.setLowerBound(this.lb);
			genP1.setUpperBound(this.ub);
			return 1- relativeDistanceMaxOverlap(g, genP1);

		}else{
			genP1 = new Gene();
			genP1.setLowerBound(this.lb);
			genP1.setUpperBound(this.ub);
			return relativeDistanceMaxOverlap(g, genP1);

		}
	}

	public double relativeDistanceMaxOverlap(Gene g, Gene gen){
		...
		if((gen.ub == gen.lb) || (g.ub == g.lb)){ // cuando no representan un intervalo sino son un punto porque lb == ub
			...
			else relativeDist= Math.max((Math.min(gen.ub,g.ub) - Math.max(gen.lb,g.lb))/(gen.ub - gen.lb), (Math.min(gen.ub,g.ub) - Math.max(gen.lb,g.lb))/(g.ub - g.lb));
		}
