...
public class Poblacion {
	...
	public void Initialize(int tam1, double [][]v1, double[]s1,int tam2,double [] []v2,double[] s2,int n_variables,int reglas, int var, double sal,double []v, long semilla){
		...
		THRESHOLDR = (double) (n_reglas_total/4.0);
		reduccionIni = THRESHOLD * 0.001;
		reduccionR = THRESHOLDR * 0.1;
		...
		BEST_CROM= new Cromosoma(Genes,GenesA,n_reglas_total);
		...
		for (j = 0; j < GenesA; j++){
		   	BEST_CROM.set_geneA(j,Gene[j].min() + (Gene[j].max() - Gene[j].min())/2.0);
			Poblacion[0].set_geneA(j, BEST_CROM.gene(j));
		}
		for (j = 0; j < n_reglas_total; j++){
		   	BEST_CROM.set_geneR(j, 1);
			Poblacion[0].set_geneR(j, 1);
		}
			for (i=1; i < Popsize; i++){
				for (j=0; j<n_reglas_total; j++){  
		        	int auxi = Randomize.Randint(0,1);
		        		Poblacion[i].set_geneR(j, auxi);
				}	
		     for (j = 0; j < Genes; j++)
	...
	public void ReStart(){
		...
		if (BEST_CROM.entrado()==1) {
			...
			for (j=0; j<n_reglas_total; j++)  Poblacion[0].set_geneR(j, BEST_CROM.geneR(j));
	   ...
	   for (j=0; j<n_reglas_total; j++)
		 Poblacion[i].set_geneR(j, BEST_CROM.geneR(j)); 
	}
		 THRESHOLD = (double) ((Genes+GenesA) * BITS_GEN / 4.0);
		 THRESHOLDR = (double) (n_reglas_total / 4.0);
		 reduccionIni = THRESHOLD * 0.001;
		 reduccionR = THRESHOLDR * 0.1;;
			...
			if (BEST_CROM.entrado()==1) {
				...
				for (j=0; j<Genes; j++)  Poblacion[0].set_gene(j, BEST_CROM.gene(j));
				for (j=0; j<GenesA; j++)  Poblacion[0].set_geneA(j, BEST_CROM.geneA(j));
				for (j=0; j<n_reglas_total; j++)  Poblacion[0].set_geneR(j,  1);
				Poblacion[0].set_perf(BEST_CROM.perf());
	 ...
	 * @param js a vector which is need for calculate the distance
	 * @param js2 the other vector which compare with Cr_1 to get the distance
	 * @param genes the size of the vector
	 * @return the number of different values between the vectors
	 */
	 private	int DistHam2(int[] js, int[] js2,int genes){
	 ...
	 /**
		 ...
		 * @return the number of different values between the vectors
		 */
		 private	int DistHam(char[] js, char[] js2,int genes){
	...
	void HUX(int[]is, int[]is2, int[]is3,  int[]is4){
	   ...
	   if (distintos>0 && intercambios==0)
		...
		for (j = 0; j < intercambios; j++) {
	      ...
	      temp = is3[posiciones[pos]];
	      is3[posiciones[pos]] = is4[posiciones[pos]];
	      is4[posiciones[pos]] = temp;
		  ...
		  for (i=0; i<Popsize/2; i++)
		    {
		      ...
		      distancia = DistHam2(Poblacion[mom].GeneR(), Poblacion[dad].GeneR(), n_reglas_total);
		  	  distancia /= 2.0;
		 if (distancia > THRESHOLDR && THRESHOLDR >= 0.0) {
		  		  HUX(Poblacion[mom].GeneR(), Poblacion[dad].GeneR(), Poblacion[POPSIZE].GeneR(), Poblacion[POPSIZE+1].GeneR());
		  		  cruce=1;
		  	 }
		  	 else{
		  		 for (j=0; j<n_reglas_total; j++) {
		  	        Poblacion[POPSIZE].set_geneR(j, Poblacion[mom].geneR(j));
		  	        Poblacion[POPSIZE+1].set_geneR(j, Poblacion[dad].geneR(j));
		  		  }
		  	      noCruceR++;
		  	 }
		    ...
		    double dist1=DistHam(String1, String2,GenesA*BITS_GEN);
		      ...
		      if (distancia > (THRESHOLD)){
		        ...
		        cruce=1;
		    	  ...
		    	  for (j=0; j<Genes; j++) {
		    	        Poblacion[POPSIZE].set_gene(j, Poblacion[mom].gene(j));
		    	        Poblacion[POPSIZE+1].set_gene(j, Poblacion[dad].gene(j));
		    		  }
		    		  for (j=0; j<GenesA; j++) {
		    	        Poblacion[POPSIZE].set_geneA(j, Poblacion[mom].geneA(j));
		    	        Poblacion[POPSIZE+1].set_geneA(j, Poblacion[dad].geneA(j));
		    		  }
		    	      noCruce++;
		    		}
		      if (cruce>0) {
...
public double getReduccionR() {
	return reduccionR;
}
public void setReduccionR(double e) {
	reduccionR=e;
}
