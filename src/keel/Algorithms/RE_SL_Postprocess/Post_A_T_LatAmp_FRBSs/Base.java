...
void EmpMasCercanas (double [] entradas) {
	 ...
	 */
	   ...
	   return (WECOA ());
		 ...
		 */
	void Decodifica (double [] cromosoma, double [] cromosomaA,char [] cromosomaR,int n_reglas_total)
	/* Pasa la Base de Conocimiento codificada en el cromosoma a una estructura
	   adecuada para inferir */
	{
	   int i, j, pos, posA;
	   double desplazamiento=-1.;
	   double variacionSoporte;
	   Difuso [][] baseaux;
	  baseaux= BDatos;
	
	   n_reglas=0;
	   for (i=0, pos=0, posA=0; i<n_reglas_total; i++)
	      ...
	      for (j=0; j<n_variables; j++) {
	          ...
	          B[n_reglas][j].sety(1.0);//[i][j].y = 1.0;
		         if (Bregla[index[i]][j] == 0){
		        	...
		        	if (j<n_var_estado) {
		      		  variacionSoporte = (cromosomaA[posA]-0.5) * (baseaux[j][Bregla[index[i]][j]].x3() - baseaux[j][Bregla[index[i]][j]].x1());
