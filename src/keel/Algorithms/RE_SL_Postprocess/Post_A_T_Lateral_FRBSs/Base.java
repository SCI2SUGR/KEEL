...
public class Base {
	...
	public Base(int variables,int reglas,int nvarstado,double salida,double []base){
		...
		int m=0;
		for (int i=0;i<n_reglas;i++){
			for(int j=0;j<n_variables;j++){//cojo fichero entrada //cada valor de una columna
...
void EmpMasCercanas (double [] entradas) {
		 ...
		 double []ents=new double[100];
		   int i, j, first, second;

		   if (n_var_estado > 100)
		   	System.exit(1);

			rango(); /* Calculo del rango para normalización de las distancias */

			/* Busqueda de las dos reglas más cercanas */
		   first = 0;
		   	...
		   	if (entradas[j] <= B[first][j].x0())
		      	...
		      	if (entradas[j] >= B[first][j].x3())
		         	...
		         	if (entradas[j] <= B[second][j].x1()) {
		            	...
		            	ents[j] = B[first][j].x0() + (B[first][j].x3()-B[first][j].x0())*0.1;
							if (ents[j] > entradas[j]) /* Evito que el punto se aleje */
		               	ents[j] = entradas[j];
		            }
		      /*printf (" + %f", ents[j]);
		   ...
		   for (i = 0; i<n_reglas; i++) {
		      if (GradoEmp[i] != 0.0) {
		         if (GradoEmp[i] == 1.0){
		        	 ...
		        	 Consecuentes[i].setx1(B[i][n_variables-1].x1());
		        	 Consecuentes[i].setx2(B[i][n_variables-1].x2());
		        	 Consecuentes[i].setx3(B[i][n_variables-1].x3());
		        	 Consecuentes[i].sety(B[i][n_variables-1].y());
		        	 Consecuentes[i].setml(B[i][n_variables-1].ml());
		         }
	          else {
		            Consecuentes[i].setx0( B[i][n_variables-1].x0());
		            Consecuentes[i].setx1 ( B[i][n_variables-1].x0() + (B[i][n_variables-1].x1()-
					                        B[i][n_variables-1].x0())*GradoEmp[i]);
		            Consecuentes[i].setx2 ( B[i][n_variables-1].x3() + (B[i][n_variables-1].x2()-
					                        B[i][n_variables-1].x3())*GradoEmp[i]);
		            Consecuentes[i].setx3(B[i][n_variables-1].x3());
		         }
		      }
		   ...
		   for (i=0; i<n_reglas; i++)
		      if (Consecuentes[i].y() != 0.0) {
		         num += B[i][n_variables-1].y() * GradoEmp[i] *
		               (Consecuentes[i].AreaTrapecioX()/
		                Consecuentes[i].AreaTrapecio ());
	   ...
	   adecuada para inferir */
	{
		int i, j, pos;
		   ...
		   for (i=0, pos=0; i<n_reglas; i++)
		      for (j=0; j<n_variables; j++) {
		    	  B[i][j].setx0(BDatos[j][Bregla[index[i]][j]].x0());
		    	  B[i][j].setx1(BDatos[j][Bregla[index[i]][j]].x1());
		    	  B[i][j].setx2(BDatos[j][Bregla[index[i]][j]].x2());
		    	  B[i][j].setx3(BDatos[j][Bregla[index[i]][j]].x3());
		    	  B[i][j].setml(BDatos[j][Bregla[index[i]][j]].ml());  
		          B[i][j].sety(1.0);
		       
		         if (Bregla[index[i]][j] == 0){
		         	desplazamiento = (cromosoma[pos]-0.5)
		            	* (BDatos[j][Bregla[index[i]][j]+1].x1() - BDatos[j][Bregla[index[i]][j]].x1());
		         }if (Bregla[index[i]][j] == (n_etiquetas[j]-1)){
		        	 desplazamiento = (cromosoma[pos]-0.5)
		            	* (BDatos[j][Bregla[index[i]][j]].x1() - BDatos[j][Bregla[index[i]][j]-1].x1());
		         ...
		         B[i][j].setx0( B[i][j].x0() + desplazamiento);
		         B[i][j].setx1( B[i][j].x1() + desplazamiento); 
		         B[i][j].setx2( B[i][j].x1());
		         B[i][j].setx3( B[i][j].x3() + desplazamiento);
					pos++;
			}
		}
	...
	public void setBDatos(int j, int k, Difuso difuso) {
		...
		BDatos[j][k].setx0(difuso.x0());
		BDatos[j][k].setx1(difuso.x1());
		BDatos[j][k].setx2(difuso.x2());
		BDatos[j][k].setx3(difuso.x3());
		BDatos[j][k].sety(difuso.y());
		BDatos[j][k].setml(difuso.ml());
