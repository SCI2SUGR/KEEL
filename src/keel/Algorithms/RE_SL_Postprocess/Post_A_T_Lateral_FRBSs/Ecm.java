...
public class Ecm {
	...
	public double ECM_tra (double [] cromosoma){
	   ...
	   for (i=0,suma=0.0; i<long_tabla_tra; i++){
	      ...
	      suma += 0.5 * Math.pow (tabla_tra[i].ejemplo[Bs.getN_var_estado()]-aux,2.0);
	  // System.out.println(tabla_tra[i].ejemplo[Bs.getN_var_estado()]+" "+Bs.FLC (tabla_tra[i].ejemplo,i)+" "+suma);
	...
	public double ECM_tst (double [] cromosoma){
	   ...
	   for (i=0,suma=0.0; i<long_tabla_tst; i++){
		   double aux=Bs.FLC(tabla_tst[i].ejemplo);
	      suma += 0.5 * Math.pow (tabla_tst[i].ejemplo[Bs.getN_var_estado()]-aux,2.0);
	   }
	...
	public double eval_EC (double []cromosoma)
	{
	   return (ECM_tra (cromosoma));
	}
}
