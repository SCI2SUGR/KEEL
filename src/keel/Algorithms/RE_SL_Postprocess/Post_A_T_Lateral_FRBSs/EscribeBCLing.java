		 ...
		 output += B.getN_reglas()+"\n"+"\n";
		   for (i=0; i<B.getN_reglas(); i++) {
			 ...
			 /*  output+="\nBase de Datos inicial: \n\n";
			   ...
			   for (i=0,k=0;i<B.getN_variables();i++) {
						output+="  Variable "+(i+1)+"\n";
				   	for (j=0;j<B.getN_etiquetas(i);j++)
				   		output+="    Etiqueta "+(j+1)+" ( "+ B.getBDatos_x0(i, j)+ " , "+B.getBDatos_x1(i, j)+" , "+B.getBDatos_x3(i, j)
				         +" )   LAT : "+ (P.getPoblacion(i).gene(k++)-0.5)+"\n";
