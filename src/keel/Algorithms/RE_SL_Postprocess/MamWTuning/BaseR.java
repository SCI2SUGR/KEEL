	...
	public void leer_BR (String fichero){
		...
		for (i=0; i<n_reglas;  i++) {
			for (j=0; j<tabla.n_variables; j++) {
				...
				BaseReglas[i][j].ml = 1.0;
				
				k = repetida = 0;
				while (k<i && repetida==0) {
					if (BaseReglas[i][j].x0==BaseReglas[k][j].x0 && BaseReglas[i][j].x1==BaseReglas[k][j].x1 && BaseReglas[i][j].x3==BaseReglas[k][j].x3)
						repetida=1;
					else  k++;
				}
				if (repetida==0)  n_etiq_distintas++;

				BaseReglas[i][j].Etiqueta = "E" + j + i;
				BaseReglas[i][j].Nombre = "x" + j;
			}
		}
	}
  ...
  ------------------------------------------------------------------------- */
	...
	public void Min (double [] entradas) {
		...
		for (b=0; b<n_reglas; b++) {
			...
			for (b2=1; minimo>0 && b2<tabla.n_var_estado; b2++) {
	...
	public double WECOA () {
		...
		for (i=0; i < n_reglas; i++)
			if (Consecuentes[i].y!=0) {
				num += BaseReglas[i][tabla.n_var_estado].y * GradoEmp[i] * (AreaTrapecioX (Consecuentes[i].x0,Consecuentes[i].x1, Consecuentes[i].x2,Consecuentes[i].x3,Consecuentes[i].y) / AreaTrapecio (Consecuentes[i].x0,Consecuentes[i].x1, Consecuentes[i].x2,Consecuentes[i].x3,Consecuentes[i].y));
				den += BaseReglas[i][tabla.n_var_estado].y * GradoEmp[i];
			}
  ...
  ------------------------------------------------------------------------- */
	...
	public String BRtoString () {
		...
		for (i=0; i < n_reglas; i++) {
			...
			cadena += "" + BaseReglas[i][tabla.n_var_estado].y + "\n";
			cadena += "\n";
		}
