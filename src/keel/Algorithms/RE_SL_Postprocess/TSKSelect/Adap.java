...
class Adap {
	...
	public Adap (MiDataset training, MiDataset test, BaseR base, BaseR base_t, int genes, double porc_radio_nicho, double porc_min_reglas, int n_soluciones, double valor_tau, double valor_alfa, int tipo) {
		int i;
		...
		for (i=0; i<base_total.n_reglas; i++) {
			if (cromosoma[i]=='1') {
				for (j=0; j<tabla.n_var_estado; j++) {
					...
					base_reglas.BaseReglas[base_reglas.n_reglas].Ant[j].Nombre = base_total.BaseReglas[i].Ant[j].Nombre;
					base_reglas.BaseReglas[base_reglas.n_reglas].Ant[j].Etiqueta = base_total.BaseReglas[i].Ant[j].Etiqueta;
				}
...
/* ---------------------------- Funcion fitness --------------------------- */
	...
	double eval_EC (char [] cromosoma) {
		...
		if (mincb >=tau && min_reglas<=base_reglas.n_reglas) {
			...
			if (Pen_nicho != 2.0)  fitness = ec * Pen_nicho;
  ...
  ------------------------------------------------------------------------- */
	...
	double P (char [] cromosoma) {
		...
		for (i=0; i<cont_soluciones; i++) {
			...
			if (dist < peor_dist)  peor_dist = dist;
		}
