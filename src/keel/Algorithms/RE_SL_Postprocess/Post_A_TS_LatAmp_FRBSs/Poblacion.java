...
public class Poblacion {
	...
	public void Initialize(int tam1, double [][]v1, double[]s1,int tam2,double [] []v2,double[] s2,int n_variables,int reglas, int var, double sal,double []v, long semilla){
		...
		GenesA=E.base().getN_reglas()*E.base().getN_var_estado();
		Ajuste =1;
		Gene = new Gen[Genes];
	...
	public void Evaluate(int inicio, int fin){	
       ...
       for (i=inicio; i<fin; i++) {
    	   Poblacion[i].set_perf (E.eval_EC(Poblacion[i].Gene(),Poblacion[i].GeneA(),Poblacion[i].GeneR(),n_reglas_total)); 
	...
	*/
	void HUX(char []Padre1, char []Padre2, char []Hijo1,  char []Hijo2){
	   ...
	   if (distintos>0 && intercambios==0)//!intercambios
	   	intercambios = 1;
		  ...
		  for (i=0; i<Popsize/2; i++)
		    {
		      ...
		      distancia = DistHam(Poblacion[mom].GeneR(), Poblacion[dad].GeneR(), n_reglas_total);
		  	  distancia /= 2.0;
