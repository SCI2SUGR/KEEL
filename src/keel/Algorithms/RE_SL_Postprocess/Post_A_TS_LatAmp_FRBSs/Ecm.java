...
public class Ecm {
			   ...
			   for (j=0; j<Bs.getN_variables(); j++) {
			      ...
			      num_etiq = Bs.getN_etiquetas(j);
				   ...
				   for (;extremos[j].min() < Dtemp.x0()+ancho*0.05;) {
					   		Bs.setN_etiquetas(j, Bs.getN_etiquetas(j)+1);
					   		Bs.setBDatos(j,Bs.getN_etiquetas(j),new Difuso());
	 ...
	 */
	public double ECM_tra (double [] cromosoma, double []cromosomaA, char[]cromosomaR ,int n_reglas_total){
	   int i;
	   double suma;
	 ...
	 * @return the mean square error
	 */
	public double ECM_tst (double [] cromosoma, double []cromosomaA, char []cromosomaR,int n_reglas_total){
	   int i;
	   double suma;
