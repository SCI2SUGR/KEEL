...
public class Ecm {
				   ...
				   //Difuso Dtemp=new Difuso();
					for (i=0; i<Bs.getN_reglas(); i++) {
						for (j=0; j<n_variables; j++) {
							...
							desp = (((Difuso) Bs.getBDatos(j,1)).x1() - ((Difuso) Bs.getBDatos(j,0)).x1()) * Bs.BaseC_desp[i][j];

							Bs.setB(i,j, Bs.getBDatos(j,(int) Bs.getBregla(Bs.getIndex(i),j)));
							Bs.getB(i, j).setx0(Bs.getB(i,j).x0()+desp);
							Bs.getB(i, j).setx1(Bs.getB(i,j).x1()+desp);
							Bs.getB(i, j).setx2(Bs.getB(i,j).x1());
							Bs.getB(i, j).setx3(Bs.getB(i,j).x3()+desp);
						}		 
					}
			   }
}
	 ...
	 * @param n_reglas_total it contains the number of total rules
	...
	public double ECM_tra (double [] cromosoma, char []cromosomaR ,int n_reglas_total){
	   ...
	   Bs.Decodifica(cromosoma,cromosomaR,n_reglas_total);
	 ...
	 * @return the mean square error(MSE)
	 */
	public double ECM_tst (double [] cromosoma,  char []cromosomaR,int n_reglas_total){
	   int i;
	   double suma;
