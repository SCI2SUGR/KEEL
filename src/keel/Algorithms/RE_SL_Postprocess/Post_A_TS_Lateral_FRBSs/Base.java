...
public class Base {
	...
	public Base(int variables,int reglas,int nvarstado,double salida,double []base){
		...
		this.BaseC_desp = new double[n_reglas][];
		for(int i1=0;i1<n_reglas;i1++){
			this.BaseC_desp[i1] = new double[n_variables];
	...
	void rango () {
		...
		int i, j;
			...
			rangos[j].set_max(B[0][j].x1()); //BaseC[0][j].x1;
...
void EmpMasCercanas (double [] entradas) {
	 ...
	 */
	   ...
	   if (Min (Entrada)>0) {
		 ...
		 */
	void Decodifica (double [] cromosoma, char [] cromosomaR,int n_reglas_total)
	  ...
	  char aux = (char)1;
	   ...
	   for (i=0,pos=0; i<n_reglas_total; i++){
	      ...
	      for (j=0; j<n_variables; j++) {
	          ...
	          B[n_reglas][j].sety(1.0);//[i][j].y = 1.0;
		         if (Bregla[index[i]][j] == 0){
		         	desplazamiento = (cromosoma[pos]-0.5)* 
		         	(baseaux[j][Bregla[index[i]][j]+1].x1() - baseaux[j][Bregla[index[i]][j]].x1());
		         }
		         	if (Bregla[index[i]][j] == (n_etiquetas[j]-1)){
		         	desplazamiento = (cromosoma[pos]-0.5)* 
		         	(baseaux[j][Bregla[index[i]][j]].x1() - baseaux[j][Bregla[index[i]][j]-1].x1());
		         	}
		     ...
		     B[n_reglas][j].setx0(B[n_reglas][j].x0()+ desplazamiento);
	         B[n_reglas][j].setx1(B[n_reglas][j].x1()+ desplazamiento);
	         B[n_reglas][j].setx3(B[n_reglas][j].x3()+ desplazamiento);
	...
	public Difuso getBDatos(int j, int k) {
	...
	public void setB(int i, int j, Difuso difuso) {
		B[i][j]=new Difuso();
		B[i][j].set_difuso(difuso);
	}
