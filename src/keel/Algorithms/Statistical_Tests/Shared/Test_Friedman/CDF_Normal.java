...
public class CDF_Normal extends Object {
	...
	*/
	 ...
	 *
	 *<p>
	 *This method calculates the  inverse normal cumulative distribution function.
	 *Let PHI(x) be the normal cdf.  Suppose that Q calculates
	 *1.0 - PHI(x), and that QINV calculates QINV(p) for p in (0.0,.5].
	 *Then for p <= .5, x = PHIINV(p) = -QINV(p).
	 *For p > .5, x = PHIINV(p) = QINV(1.0 - p).
	 ...
	 *</p>
	 *@param   p    A double, x value, must lie between 0 and 1.  xnormi returns
	 *              
	 *@return The inverse normal cumulative distribution function evaluated at p.
    ...
    * </p>
    *@param   z   x value
    * @return The value value of the normal cumulative distribution function at z
    */
