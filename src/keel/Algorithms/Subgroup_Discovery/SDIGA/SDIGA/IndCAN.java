...
public class IndCAN  extends Individual {
    ...
    public void RndInitInd(TableVar Variables) {
      cromosoma.initCrom(Variables);  // Random initialization method
        ...
        for (int i=0; i<Examples.getNEx(); i++) { // For each example of the dataset
            ...
            if (disparoFuzzy>0) {
                ...
                if ((!Examples.getCovered(i)) &&  (Examples.getClass(i) == Variables.getNumClassObj())) {
                    // If example was not previously covered and belongs to the target class increments the number of covered examples
                    ejCompAntClassNewFuzzy++;
                    gradoCompAntClassNewEjFuzzy += disparoFuzzy;
                    if (marcar && AG.getObj1().compareTo("CSUP")!=0)
