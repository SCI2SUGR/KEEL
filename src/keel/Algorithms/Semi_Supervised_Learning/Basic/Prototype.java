...
public class Prototype
{
    ...
    /** weight of instance. By default is 1 **/
    ...
    public Prototype getPrototypeWithSelectedInputs(ArrayList<Integer> lista)
    {
    	...
    	double [] reducido = new double[lista.size()];
    	
    	for(int i=0; i<lista.size(); i++){
    		reducido[i] = this.getInput(lista.get(i));
    	}
    	
    	Prototype otro = new Prototype(reducido,this.outputs);
...
//              System.out.println(comma[1]);
    ...
    public Prototype denormalize()
    {
        ...
        for(int i=0; i<nInputs; ++i)
        {
        	...
        	if(value>max_inputs[i]) {value= max_inputs[i];}
        	else if(value<min_inputs[i]) {value = min_inputs[i];}
    ...
    public String getInputAsNominal(int i)
    {
        ...
        {
        	//System.out.println("denormalizao");
            //Como ya estÃ¡ desnormalizado, sÃ³lo extraemos el Ã­ndice del atributo nominal
        	
            indexOfNominalAttr = (int)(Math.round(inputs[i]));
            //indexOfNominalAttr = Attributes.getInputAttribute(i).getNumNominalValues()-1;
        }
        
     //  System.out.println("Input attribute = "+i); //", "+Attributes.getInputAttribute(i).)
      //  System.out.println(Attributes.getInputAttribute(i).getNumNominalValues());
    
       // System.out.println(indexOfNominalAttr);
    ...
    public String getOutputAsNominal(int i)
    {
        ...
        int _index = (int)(Math.round(outputs[i]));// Attributes.getOutputAttribute(i).getNumNominalValues()-1;// 
