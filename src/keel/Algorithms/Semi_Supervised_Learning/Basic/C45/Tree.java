   ...
   */
	...
	public ArrayList <Integer> leafsSize() {
		...
		if (sons != null) {
			for (int i = 0; i < sons.length; i++) {
				if (sons[i].isLeaf) {
					leafSize.add(sons[i].covered);
				}
				else {
					leafSize.addAll(sons[i].leafsSize());
				}
			}
		}
	...
	public int classifyingLeaf (Itemset itemset) {
        ...
        if (isLeaf) {
            ...
            if (treeIndex == -1) {
            	System.err.println("This C4.5 version does not work with missing values");
            	System.exit(-1);
            	return -1;
            } else {
                if (son(treeIndex).isEmpty) {
                    ...
                    return son(treeIndex).classifyingLeaf(itemset);
                }
            }
        }
	}
