...
/**
* <p>
* @author Written by Cristobal Romero (Universidad de Cï¿½rdoba) 10/10/2007
...
public class Dataset {
    ...
    public Dataset(String name, boolean train) {
        ...
        // Compute the weights accordingly to the itemsets
        computeItemsetWeights();
    }
     ...
     * 
     */
    public void computeItemsetWeights () {
        // Compute the weights accordingly to the itemsets
        ...
        int freqClasses[] = new int[numClasses];
        Arrays.fill(freqClasses, 0);

        // Compute the class frequency
        for (int i = 0; i < numItemsets(); i++) {
            freqClasses[(int)(itemset(i).getClassValue())]++;
        }
        ...
        // Compute the weights for the itemsets
        positive_weight = positive_cost*((double)numItemsets()/((positive_cost*freq_pos)+(negative_cost*freq_neg)));
        negative_weight = negative_cost*((double)numItemsets()/((positive_cost*freq_pos)+(negative_cost*freq_neg)));
        
    	// Change the weights of the itemsets in the dataset
        for (int i = 0; i < numItemsets(); i++) {
        	class_itemset = (int)(itemset(i).getClassValue());
        	
        	if (class_itemset == positive_class)
        		itemset(i).setWeight(positive_weight);
        	else
        		itemset(i).setWeight(negative_weight);
        }
    }
     ...
     * 
     * @return the cost of a positive instance
     */
    public double positive_cost () {
        // Compute the cost for a positive_class
    	int numClasses = numClasses();
        int positive_class; 
        double positive_cost;
    	int freq_pos = 0;
     ...
     * 
     * @return the index of the positive class
     */
    public int positive_class () {
        // Compute the weights accordingly to the itemsets
    	int numClasses = numClasses();
        double minimum = 0;
        int minIndex = 0;
