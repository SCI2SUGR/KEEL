...
public final class M5TreeNode implements Serializable {
    ...
    M5Instances instances; // instances reaching this node
     ...
     */
    public M5TreeNode(M5Instances inst, M5TreeNode up) {
        int i;
    ...
    public final String treeToString(int treeLevel, double deviation) {
        ...
        if (type == true) {
            ...
            if (instances.attribute(splitAttr).name().charAt(0) != '[') {
                ...
                text.append(instances.attribute(splitAttr).name() + " false : ");
            }
            treeLevel++;
            ...
            if (instances.attribute(splitAttr).name().charAt(0) != '[') {
                ...
                text.append(instances.attribute(splitAttr).name() + " true : ");
            }
            treeLevel++;
    ...
    public final String formulaeToString(boolean smooth) throws Exception {
        ...
        if (type == true) {
            ...
            startingPoint = 6 + (int) (Math.log(lm + 0.5) / Math.log(10)) + 1 +
                            3;
            if (smooth == true) {
                text.append(smoothed.toString(instances, startingPoint));
            } else {
                text.append(unsmoothed.toString(instances, startingPoint));
            }
            text.append("\n");
        }
     ...
     * @exception Exception if something goes wrong
     */
    public final void split(M5Instances inst) throws Exception {
        SplitInfo s, sMax;
        int j, partition;
        M5Instances leftInst, rightInst;

        instances = inst;
        if (instances.numInstances() < SPLIT_NUM ||
            M5.stdDev(instances.classIndex(), instances) < deviation * 0.05) {
            type = false;
        } else {
            ...
            if (type == true) {
                ...
                leftInst = new M5Instances(instances, instances.numInstances());
                rightInst = new M5Instances(instances, instances.numInstances());
                ...
                if (model != REGRESSION_TREE) {
                    unsmoothed = Function.combine(unsmoothed,
                                                  leftNode.unsmoothed);
                    // passes up the attributes found under the left node
                    unsmoothed = Function.combine(unsmoothed,
                                                  rightNode.unsmoothed);
    ...
    public final void prune() throws Exception {
        ...
        if (type == false) { // LEAF
            ...
            if (model == LINEAR_REGRESSION) { // outputs linear regression model
                ...
                if (function.terms[0] <
                    Math.sqrt(instances.numInstances()) * 2.0 &&
                    function.terms[0] < 50) {
                    unsmoothed = function;
                }
    ...
    public final void function() throws Exception {
        ...
        if (f1.terms[0] != 0) {
            sdy = M5.stdDev(instances.classIndex(), instances);
            this.regression(f1);
            valueNode = false;
                ...
                while (flag == 0) {
                    ...
                    if (jmin == -1) {
                        ...
                        err2 = e2.rootMeanSqrErr *
                               this.factor(instances.numInstances(),
     ...
     * @exception Exception if something goes wrong
     */
    public final String predictionsToString(M5Instances inst, int lmNo,
            ...
            if (lmNo == 0 || lmNo == lmNum) {
                ...
                if (inst.instance(i).classIsMissing() == false) {
                    text.append("      Actual value: " +
                                M5.doubleToStringG(inst.instance(i).classValue(),
                            9, 4) +
                                "    Prediction: " +
                                M5.doubleToStringG(value, 9, 4) +
                                "    Abs. error: " +
                                M5.doubleToStringG(Math.
                            abs(inst.instance(i).classValue() - value), 9, 4) +
                                "\n\n");
                } else {
    ...
    public final int leafNum(M5Instance instance) {
        ...
        if (type == false) {
            ...
            if (instance.value(splitAttr) <= splitValue) {
                ...
                lmNum = rightNode.leafNum(instance);
            }
        }
    ...
    public final double predict(M5Instance instance, boolean smooth) {
        ...
        if (type == false) { // LEAF
            ...
            if (instance.value(splitAttr) <= splitValue) {
                ...
                y = rightNode.predict(instance, smooth);
            }
        }
     ...
     * @exception Exception if something goes wrong
     */
    public final Results errors(M5Instances inst, boolean smooth) throws
            Exception {
        int i;
        double tmp;
        ...
        for (i = 0; i <= inst.numInstances() - 1; i++) {
            tmp = this.predict(inst.instance(i), smooth) -
                  inst.instance(i).classValue();
            e.sumErr += tmp;
     ...
     * @exception Exception if something goes wrong
     */
    public final Measures measures(M5Instances inst, boolean smooth) throws
            Exception {
        ...
        measures.correlation = M5.correlation(y1, y2, numInstances);

        sd = M5.stdDev(inst.classIndex(), inst);
        if (sd > 0.0) {
    ...
    public final M5TreeNode copy(M5TreeNode up) throws Exception {
        ...
        M5TreeNode node = new M5TreeNode(instances, upNode);
        ...
        node.instances = new M5Instances(instances, 0,
     ...
     * @exception Exception if something goes wrong
     */
    public final String measuresToString(Measures measures[], M5Instances
        ...
        absDev = M5.absDev(inst.classIndex(), inst);
        sd = M5.stdDev(inst.classIndex(), inst);
