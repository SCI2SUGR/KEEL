...
public class Main {
    ...
    private void preparaArgumentos(String nomFichero) {
        ...
        datos = new StringTokenizer(una_linea, " = \" ");
        datos.nextToken();
        filter = datos.nextToken();
        una_linea = linea.nextToken();
        datos = new StringTokenizer(una_linea, " = \" ");
        datos.nextToken();
        kernel = datos.nextToken();
    };
    ...
    private void execute() {
        MISMO mi_mismo = new MISMO(ficheroTrain, ficheroEval, ficheroTest,
                          ficheroSalidatr, ficheroSalidatst, ficheroSalida,
                          semilla, filter, kernel);
        if (mi_mismo.everythingOK()) {
            mi_mismo.execute();
        }
    }
