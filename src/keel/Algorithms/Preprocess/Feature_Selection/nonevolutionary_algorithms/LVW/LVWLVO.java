...
public class LVWLVO {
	 ...
	 *
	 * This class implements LVW algorithm. It selects a random subset of features and checks
	 * its precision.
	 * </p>
	 */
     ...
     */
    public LVWLVO(String ficParametros) {
    ...
    /**
     * <p>
     * Main method for LVWLVO. Generates random subsets and checks their precision using
     * a wrapper method, the LVO Classifier. Finally, it returns the subset with the lowest
     * error
     * </p>
     */
    private void lanzarLVW(){
        boolean featuresVector[];
        int i;
        double errorActual, mejorError = 0;
        
        /* firstly, sets the initial best error */
        featuresVector = new boolean[data.returnNumFeatures()];
        for(i=0; i<featuresVector.length; i++)
            featuresVector[i] = true;

        mejorError = data.LVO(featuresVector);
        features = featuresVector;
        
        i = 0;
        while(i < params.maxLoops){
            /* generates a random subset of selected features */
            featuresVector = generarCtoAleatorio();
            
            /* obtains the precision of the current subset using LVO Classifier */
            errorActual = data.LVO(featuresVector);
            
            if(errorActual < mejorError){
                /* refreshs the best subset and its error */
                features = featuresVector;
