	...
	/**
       ...
       */
        ...
        for(int i=0; i<params.numInstancesSampled; i++){
            ...
            /* returns the position of the miss nearest neighbour */
            posJ = data.findNearestMiss(posI);
            
            /* calculates features's weights, and adds to an array */
            for(int j=0; j<w.length; j++)
                w[j] = w[j] - Math.pow(data.diff(j, posI, posH), 2)/params.numInstancesSampled 
                        + Math.pow(data.diff(j, posI, posJ), 2)/params.numInstancesSampled; 
