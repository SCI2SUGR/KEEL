 ...
 * @author Modified by Victoria Lopez Morales (University of Granada) 17/04/2010
 * @version 0.1
 * @since JDK1.5
 *
 */

package keel.Algorithms.Preprocess.Instance_Selection.Safe_Level_SMOTE;
...
public class Safe_Level_SMOTE extends Metodo {
   ...
   */
  public Safe_Level_SMOTE (String ficheroScript) {
    super (ficheroScript);
  }
  ...
  public void run () {
    ...
    int slp, sln;
    double sl_ratio;
    int need_to_generate, generated;
    ...
    need_to_generate = genS.length;
    generated = 0;
    for (i=0; i<need_to_generate; i++) {
    	clasesGen[generated] = posID;
    	nn = Randomize.Randint(0,kSMOTE-1);
    	
    	slp = 0;
    	sln = 0;
    	
    	for (int v=0; v<kSMOTE; v++) {
    	    if (clasesTrain[neighbors[i%positives.length][nn]] == posID) {
    	        slp++;
    	    }
    	    else {
    	        sln++;
    	...
    	if (sln != 0) { // sl is safe level
    	    sl_ratio = (double)slp/(double)sln; // sl_ratio is safe level ratio
    	}
    	else {
    	    sl_ratio = Double.POSITIVE_INFINITY;
    	}
    	
    	if ((sl_ratio == Double.POSITIVE_INFINITY) && (slp == 0)){
    	    // the first case
            ...
            interpolate (datosTrain[positives[i%positives.length]],datosTrain[neighbors[i%positives.length][nn]],genS[generated],interpolation,alpha,mu,sl_ratio,slp);
            generated++;
    	}
    }
   ...
   /*Construction of the S set from the previous vector S*/
    ...
    System.out.println("Safe_Level_SMOTE "+ relation + " " + (double)(System.currentTimeMillis()-tiempo)/1000.0 + "s");
     ...
     * </p>
     *
     * @param ra    Array with the values of the first example in the current population
     * @param rb    Array with the values of the second example in the current population
     * @param resR  Array with the values of the generated example
     ...
     * @param sl_ratio  Safe level of a positive instance/Safe level of a nearest neighbor
     * @param slp   Safe level of a positive instance
     */
    void interpolate (double ra[], double rb[], double resR[], String interpolation, double alpha, double mu, double sl_ratio, int slp) {
            ...
            gap = Randomize.Rand();
            if ((sl_ratio == Double.POSITIVE_INFINITY) && (slp != 0)) { // The second case
                gap = 0;
            }
            else if (sl_ratio == 1) { // The third case
                gap = Randomize.Rand();
            }
            else if (sl_ratio > 1) { // The fourth case
                gap = Randomize.Randdouble (0, 1.0/sl_ratio);
            }
            else if (sl_ratio < 1) { // The fifth case
                gap = Randomize.Randdouble (1.0-sl_ratio, 1.0);
            }
