...
public class ConjGradNN {
	...
	*/ 
    ...
    public ConjGradNN(
        ...
        for(int ne = 0 ; ne < nElements.length;++ne)
        	System.out.println("conche = " + nElements[ne]);
        ...
        inputU = new double[Input.length][nOutputs];
        outputV = new double[Input.length][nOutputs];
        classProbability = new double[Input.length][nOutputs];
        originalClass = new int[Input.length];
        finalClass = new int[Input.length];
        initialClassDistribution = new double[nOutputs];
        Arrays.fill(initialClassDistribution, 0);
        
        currentClassDistribution = new double[nOutputs];
        Arrays.fill(currentClassDistribution, 0);
        
        for(int nc = 0 ; nc < nOutputs ; ++nc)
        	System.out.println(Output[0][nc]);
        
        double probRest = 0.05 / (nOutputs-1);
        final double u0 = 0.02;
        for(int ni = 0 ; ni < Input.length ; ++ni){
        	for(int nc = 0 ; nc < nOutputs ; ++ nc){
            	
        		if(Output[ni][nc] == 1){
        			classProbability[ni][nc] = 0.95;
        			outputV[ni][nc] = 0.95;
        			originalClass[ni] = nc;
        			finalClass[ni] = nc;
        			initialClassDistribution[nc]++;
        			currentClassDistribution[nc]++;
        			inputU[ni][nc] = atanh(2*outputV[ni][nc]-1)*u0;
        		}
        		
        		else{
        			classProbability[ni][nc] = probRest;
        			outputV[ni][nc] = probRest;
        			inputU[ni][nc] = atanh(2*outputV[ni][nc]-1)*u0;
        		}
        	}
        }
        ...
        for(int nc = 0 ; nc < nOutputs ; ++nc){
        	initialClassDistribution[nc] /= Input.length;
        }
        
        //for(int kk = 0 ; kk < 9 ; ++kk){
        //	System.out.print(originalClass[kk] + ", ");
        //}
        //**********************************************************************
    ...
    */   
    ...
    private double[][][] gradient(FUN f, double x[][][]) {
	    ...
	    for (i=0;i<x.length-1;i++) {
            ...
            //**************************************************************
            // actualizar mis parametros

            // actualizar Ui
            final double Lp = 0.0005;
            for(int nc = 0 ; nc < nOutputs ; ++nc){
            	inputU[k][nc] = inputU[k][nc] + Lp*(output[nc]-classProbability[k][nc]);
            	//System.out.println("output " + nc + " = " + inputU[nc]);
            }
            
            // actualizar Vi
            final double u0 = 0.02;
            for(int nc = 0 ; nc < nOutputs ; ++nc){
            	outputV[k][nc] = 0.5*(1+Math.tanh(inputU[k][nc]/u0));
            	//System.out.println("output " + nc + " = " + outputV[nc]);
            }
            
            // actualizar pi y coger el maxmimo
            int posMAX = 0;
            double valueMAX = outputV[k][0];
            for(int nc = 1 ; nc < nOutputs ; ++nc){
            	
            	classProbability[k][nc] = outputV[k][nc];
            	
            	//System.out.println("classProb " + nc +" = " + classProbability[k][nc]);
            	
            	if(classProbability[k][nc] > valueMAX){
            		valueMAX = classProbability[k][nc]; 
            		posMAX = nc;
            	}
            }
            
    		currentClassDistribution[finalClass[k]]--;	// a su clase antigua le quito
    		currentClassDistribution[posMAX]++;			// nueva clase le sumo

    		finalClass[k] = posMAX;						// clase nueva actualizada
            ...
            // criterio de parada
            if(k % 20 == 0){
            	double sseadj = computeSSEadj();
            	if(sseadj < best_SSE_adj){
            		best_SSE_adj = sseadj;
            		contador = 0;
            	}
            	else
    ...
    public Vector getNoisyInstances(){
    	...
    	for(int i = 0 ; i < Input.length ; ++i){
    		//System.out.println(originalClass[i] + ", " + finalClass[i]);
    		if(originalClass[i] != finalClass[i])
    			res.add(i);
    }
    	return res;
    }
    ...
    public double computeSSEadj(){
        ...
        for (int i = 0 ; i < Input.length ; i++) {
            double error[] = OPV.subtract(nn(Input[i],weights),Output[i]);
            SSE_std += OPV.multiply(error,error);
        }
        // Mean square error
        //SSE_std /= Input.length;
    	
    	double SSE_hn = 0;
    	final double A1 = 0.05;
    	final double A2 = 0.2;
    	final double I = nInputs;
    	final double H = nElements[0];
    	final double N = Input.length;
    	final double C = nOutputs;
    	
    	if(H <= I)
    		SSE_hn = A1*(H-1)*N*(C-1)/C;
    	else if(H > I)
    		SSE_hn = (A1*(I-1)+A2*(H-I))*N*(C-1)/C;
    	
    	double SSE_dist = 0;
    	double primero = ( N*(C-1) ) / C;
    	double[] aux = new double[currentClassDistribution.length];
    	System.arraycopy(currentClassDistribution, 0, aux, 0, currentClassDistribution.length);
    	for(int nc = 0 ; nc < C ; ++nc){
       	...
       	for(int i = 0 ; i < C ; ++i){
    		// compute Di
       		double Di = Math.abs(aux[i]-initialClassDistribution[i]);
       		...
       		SSE_dist += Di*Bi*max(aux[i],initialClassDistribution[i]);
    	}
    	 
       	SSE_dist *= primero;
    	
    	return SSE_std+SSE_hn+SSE_dist;
    }
    //**********************************************************************************
