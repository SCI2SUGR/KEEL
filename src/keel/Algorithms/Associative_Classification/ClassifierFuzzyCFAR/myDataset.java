...
public class myDataset {
    ...
    private double[][][] Xf = null; //examples array
    ...
    public void readClassificationSet(String datasetFile, boolean train) throws
        ...
        try {
            ...
            for (int i = 0; i < nInputs; i++){
			  ...
			  if (Attributes.getInputAttribute(i).getType() == Attribute.NOMINAL)  nominal[i] = true;
			  else  nominal[i] = false;
            }
     ...
     * @param dataBase DataBase where the instances and the labels for each variables are stored and needed to transform the data.
     */
    public void transform(DataBase dataBase) {
		...
		for (i=0; i < this.nData; i++)
			for (j=0; j < this.nInputs; j++)  this.Xf[i][j] = new double[dataBase.numLabels(j)];

		for (i=0; i < this.nData; i++) {
			for (j=0; j < this.nInputs; j++) {
			    if (this.isNominal(j)) {
					for (k=0; k < dataBase.numLabels(j); k++)  this.Xf[i][j][k] = dataBase.matching(j, k, this.X[i][j]);
			    }
				else {
					total = 0.0;
					for (k=0; k < dataBase.numLabels(j); k++) {
						this.Xf[i][j][k] = dataBase.matching(j, k, this.X[i][j]);
						total += this.Xf[i][j][k];
					}

					for (k=0; k < dataBase.numLabels(j); k++)  this.Xf[i][j][k] /= total;
				}
		    }
		}
    }
    ...
    public void computeInstancesPerClass() {
        ...
        for (i = 0; i < this.getnData(); i++) {
            this.instancesCl[this.outputInteger[i]]++;
        }
    }
