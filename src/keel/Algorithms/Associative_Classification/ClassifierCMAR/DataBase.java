...
public class DataBase {
   ...
   * @param attr int Given attribute
   * @return short The sum of the number of labels until a given attribute
   */
  public short getOrderAttribute(int attr) {
	  int i;
	  short suma;

	  suma = 1;
	  for (i=0; i < attr; i++) {
		  if (this.numLabels(i) > 1)   suma += this.numLabels(i);
	  }
  ...
  public String printString() {
    ...
    for (int i = 0; i < this.n_variables; i++) {
      ...
      string += "\n" + this.names[i] + ":\n";
	  if (this.numLabels(i) > 1) {
		  for (int j = 0; j < this.nLabels[i]; j++, value++)  string += value + "\n";
	  }
	  else {
		  string += "- \n";
	  }
