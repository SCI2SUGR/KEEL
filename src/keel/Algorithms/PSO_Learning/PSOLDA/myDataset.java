 ...
 * <p>Company: KEEL </p>
 *
 * @authors Alberto Fernandez, Jose Antonio Saez Munoz
 * @version 1.0
 */
public class myDataset {
  ...
  /**
     ...
     * @param x weight vector for each attribute.
     * @param tam size of the result matrix.
     * @return The attribute values copied for each instances
     */
    public double[][] removeAttributes(double x[], int tam){
    	
    	double[][] res= new double[nData][tam];
    	int pos=0;
    	
    	for(int i=0 ; i< nData ; ++i){
    		pos=0;
    		for(int j=0 ; j<getnInputs() ; ++j){
    			if(x[j]>0.5){
    				res[i][pos]=this.X[i][j];
    				pos++;
    			}
    		}
    	}
