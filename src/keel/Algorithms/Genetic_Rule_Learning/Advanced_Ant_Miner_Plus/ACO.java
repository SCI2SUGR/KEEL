...
public class ACO {
  ...
  private float[][] feromonaAlfaBeta; //Aqui vendra la feromona para asignar dinamicamente los valores a alfa y beta
  ...
  private void creaDatos() {
    ...
    //feromona para escoger dinamicamenta alfa y beta
    feromonaAlfaBeta = new float[2][];
    for (int i = 0; i < 2; i++) {
      feromonaAlfaBeta[i] = new float[3];
    }
  ...
  public void run() {
    ...
    while (!parada) {
      ...
      while (!convergencia & indice < maxIteracionesSinConverger) {
        //escogemos dinamicamente alfa y beta
        escogeAlfaBeta();
  ...
  private void escogeAlfaBeta() {
    ...
    double probabilidadesBeta = 0;
    double denominadorAlfa = 0;
    double denominadorBeta = 0;
    double probabilidadAcumulada = 0;
    int contador = 0;

    probabilidadesAlfa = generadorA.Rand();
    probabilidadesBeta = generadorA.Rand();

    for (int i = 0; i < 3; i++) {
      denominadorAlfa += feromonaAlfaBeta[0][i] * 1 / 3;
      denominadorBeta += feromonaAlfaBeta[1][i] * 1 / 3;
    }

    while (probabilidadAcumulada < probabilidadesAlfa && contador < 3) {
      probabilidadAcumulada = (feromonaAlfaBeta[0][contador] * 1 / 3) /
          denominadorAlfa;
      if (probabilidadAcumulada < probabilidadesAlfa) {
        contador++;
      }
    }

    alfa = contador + 1; //Empieza en 0 pero no puede ser 0, como minimo 1
    if (alfa > 3) {
      alfa = 3;
    }
    probabilidadAcumulada = 0;
    contador = 0;
    while (probabilidadAcumulada < probabilidadesBeta && contador < 3) {
      probabilidadAcumulada = (feromonaAlfaBeta[1][contador] * 1 / 3) /
          denominadorBeta;
      if (probabilidadAcumulada < probabilidadesBeta) {
        contador++;
      }
    }
    beta = contador + 1; //Igual que arriba
    if (beta > 3) {
      beta = 3;
    }
...
//-------------------------------------------------------------------------------------------------------
    ...
    //Titulos
      for (i = 0; i < nombres.length - 1; i++) {
        System.out.print(nombres[i].getName() + "\t");
        salida.print(nombres[i].getName() + "\t");
      }

    System.out.println("Prediction");
    salida.println("Prediction");
    salida.println();
    System.out.println();
...
//-----------------------------------------------------------------------------------------------------
  ...
  private void aumentaFeromona(Regla hormiga) {
    ...
    //Por ultimo aumentamos la feromona de Alfa y Beta
    feromonaAlfaBeta[0][ (int) alfa - 1] += (calidad / 2);
    feromonaAlfaBeta[1][ (int) beta - 1] += (calidad / 2);
  ...
  private void inicializaFeromona() {
    ...
    for (int i = 0; i < 3; i++) {
      feromonaAlfaBeta[0][i] = maxFeromona;
    }

    for (int i = 0; i < 3; i++) {
      feromonaAlfaBeta[1][i] = maxFeromona;
    }
  }
