...
package keel.Algorithms.Genetic_Rule_Learning.C45Rules;
 ...
 */
   ...
   */
  public void classify( MyDataset trainItemsets ) throws Exception
  {
    if ( numSubsets == 1 )
    ...
    else
    {
      ...
      if ( trainItemsets.getAttribute( attributeIndex ).isDiscret() )
      {
        if ( nBranches != 2 )
        {
          nBranches = trainItemsets.getAttribute( attributeIndex ).numValues();
          nCuts = nBranches;
        }
        else
          nCuts = 0;
      ...
      else
      {
        ...
        else
        {
          ...
          for ( int i = 0; i < weights.length; i++ )
            prob += weights[i] * classification.probability( classIndex, i );
   ...
   */
  private void cutContinuous( MyDataset trainItemsets )
  {
    ...
    minCut =  0.1*( classification.getTotal() ) / ( (double)trainItemsets.numClasses() );
    ...
    while ( next < firstMiss )
    {
           ...
           + 1e-5 < trainItemsets.itemset( next ).getValue( attributeIndex ) )
      {
        ...
        {
          currentInfoGain = infoGainCutCrit(classification, sumOfWeights, defaultEnt );
    ...
    cutPoint = ( trainItemsets.itemset( cutIndex + 1 ).getValue( attributeIndex )+
                 trainItemsets.itemset( cutIndex ).getValue( attributeIndex ) ) / 2;
   ...
   * @param trainItemsets The dataset used to compute the cut.
   */
  private void cutDiscret( MyDataset trainItemsets )
  {
    Itemset itemset;
    ...
    while ( enum2.hasMoreElements() )
    {
      ...
      if ( !itemset.isMissing( attributeIndex ) )
        classification.add( (int)itemset.getValue( attributeIndex ), itemset );
    }
    ...
    if ( classification.check( minItemsets ) )
    {
      ...
      infoGain = infoGainCutCrit( classification, sumOfWeights, oldEntropy( classification )  );
      gainRatio = gainRatioCutCrit( classification, sumOfWeights, infoGain );
    }
  }
   ...
   */
  public final void setCutPoint( MyDataset allItemsets )
  {
    double newCutPoint = -Double.MAX_VALUE;
    double tempValue;
    Itemset itemset;

    if ( ( allItemsets.getAttribute( attributeIndex ).isContinuous() ) && ( numSubsets > 1 ) )
    {
   ...
   */
  public final MyDataset [] cutDataset( MyDataset data ) throws Exception
  {
    MyDataset [] itemsets = new MyDataset[numSubsets];
    double [] weights;
    double newWeight;
    Itemset itemset;
    int subset, i, j;
    ...
    for ( j = 0; j < numSubsets; j++ )
      itemsets[j] = new MyDataset( (MyDataset)data, data.numItemsets() );

    for ( i = 0; i < data.numItemsets(); i++ )
    {
      itemset = ( (MyDataset) data ).itemset( i );
   ...
   */
  public void resetClassification( MyDataset data ) throws Exception
  {
    if ( numSubsets == 1 )
      classification = new Classification( data, this );
    else
    {
      MyDataset insts = new MyDataset( data, data.numItemsets() );
    ...
    else
    {
      ...
      if ( itemset.isMissing( attributeIndex ) )
      {
        ...
        for ( i = 0; i < numSubsets; i++ )
          weights [i] = classification.perValue( i ) / classification.getTotal();
  ...
  public final double newEntropy( Classification values )
  {
    ...
    {
      for ( j = 0; j < values.numClasses(); j++ )
        returnValue = returnValue + logFunc( values.perClassPerValue( i, j ) );
   ...
   */
  public final String leftSide( MyDataset data )
  {
    if ( numSubsets == 1 )
      return "";
    else
   ...
   */
  public final String rightSide( int index, MyDataset data )
  {
    if ( numSubsets == 1 )
      return "";
    else
    {
      ...
      if ( data.getAttribute( attributeIndex ).isDiscret() )
        text.append( " = " + data.getAttribute( attributeIndex ).value( index ) );
      else if (index == 0)
   ...
   */
  public final String label( int index, MyDataset data )
  {
    StringBuffer text;

    text = new StringBuffer();
    text.append( ( (MyDataset)data ).getClassAttribute().value( classification.maxClass( index ) ) );
