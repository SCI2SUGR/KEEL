...
public final class Impurity {
     ...
     */
    public Impurity(int partition, int attribute, MyDataset inst, int k) {

        StatisticsStore values = new StatisticsStore(0, inst.numItemsets() - 1,
                inst.getClassIndex(), inst);
        attr = attribute;
        n = inst.numItemsets();
        sd = values.sd;
        va = values.va;

        values = new StatisticsStore(0, partition, inst.getClassIndex(), inst);
        nl = partition + 1;
        sl = values.sum;
        s2l = values.sqrSum;

        values = new StatisticsStore(partition + 1, inst.numItemsets() - 1,
                                     inst.getClassIndex(), inst);
        nr = inst.numItemsets() - partition - 1;
        sr = values.sum;
        s2r = values.sqrSum;
