...
public class myDataset {
    ...
    public void readClassificationSet(String datasetFile, boolean train) throws
        ...
        try {
            ...
            nameVar = new String[nInputs];
            nameValues = new String[nInputs][];
            ...
            for (int i=0; i<nInputs; i++) {
            	nameVar[i] = IS.getAttributeDefinitions().getInputAttribute(i).getName();
        		if (this.getTipo(i) == myDataset.NOMINAL) {
                	nameValues[i] = new String[IS.getAttributeDefinitions().getInputAttribute(i).getNumNominalValues()];
                	for (int j=0; j<nameValues[i].length; j++) {
                		nameValues[i][j] = IS.getAttributeDefinitions().getInputAttribute(i).getNominalValue(j);
                	}
        		} else {
                	nameValues[i] = new String[(int)IS.getAttributeDefinitions().getInputAttribute(i).getMaxAttribute() - (int)IS.getAttributeDefinitions().getInputAttribute(i).getMinAttribute() + 1];
                	for (int j=0; j<nameValues[i].length; j++) {
                		nameValues[i][j] = Integer.toString((int)IS.getAttributeDefinitions().getInputAttribute(i).getMinAttribute() + j);
                	}
        		}
     ...
     * @param pos attribute's id.
     * @return the name of the given attribute.
     */
    public String nameAttribute(int pos) {
    	return nameVar[pos];
    }

    /**
     * Returns the given value of the given attribute.
     * @param pos1 attribute's id.
     * @param pos2 value's id.
     * @return the given value.
     */
    public String valueAttribute(int pos1, int pos2) {
    	return nameValues[pos1][pos2];
    }

    /**
     * Returns the value of the given attribute of a given example.
     * @param i instance's id.
     * @param j attribute's id.
     * @return the value of the given attribute of a given example.
     */
    public int valueExample(int i, int j) {
    	return IS.getInstance(i).getInputNominalValuesInt(j);
    }
