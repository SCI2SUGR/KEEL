...
public class Rule {
	...
	public Rule(Rule regla){
		...
		limits = new double[numAttributes][2];
		for(int i=0;i<numAttributes;i++){
			active[i] = regla.active[i];
			limits[i][0] = regla.limits[i][0];
			limits[i][1] = regla.limits[i][1];
		}
		_class = regla._class;
	}
	 ...
	 * previous memory).
	 * The rules will be reset after this!
	...
	public int evaluate(double inputs[]){
		...
		for(int i=0;i<numAttributes && supported;i++){
			if(active[i] && limits[i][0]<=limits[i][1]){//use this attribute?
				...
				if(limits[i][0]>inputs[Ilga.attributeOrder[i]] || inputs[Ilga.attributeOrder[i]]>limits[i][1])
					supported = false;
			}
		}
