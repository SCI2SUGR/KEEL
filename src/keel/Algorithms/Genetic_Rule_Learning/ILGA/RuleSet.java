...
public class RuleSet implements Comparable{
	...
	public void randomizeRules(InstanceSet IS){
		...
		for(int j =0;j<nAtt;j++){
			a = Attributes.getInputAttribute(Ilga.attributeOrder[j]);
			for(int i=0;i<numberRules;i++){
...
//			rule1.setLimits(cutpoint_variable/3, rule2.getLimits(cutpoint_variable/3)[0], rule1.getLimits(cutpoint_variable/3)[1]);
	...
	public void mutate(int gene){
		...
		if(attNum<nAtt)
			a = Attributes.getInputAttribute(Ilga.attributeOrder[attNum]);
	...
	public void IS3(int whichAtt){
		...
		a = Attributes.getInputAttribute(whichAtt);
		for(int i=0;i<numberRules;i++){
			r = reglas[i];
			curClass = r.getClas();
			...
			//create a new rule with only 1 attribute
			s = new Rule(1);
			//now randomize those attributes
			s.setActivation(0, Randomize.Rand()<0.5);
			s.setLimits(0,Randomize.RanddoubleClosed(a.getMinAttribute(), a.getMaxAttribute()), Randomize.RanddoubleClosed(a.getMinAttribute(), a.getMaxAttribute()));
			r.append(s);
		}
		this.nAtt += 1;
	}
