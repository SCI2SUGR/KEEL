...
public class Dataset {
    ...
    public String[] dameClases() {
      String [] salida = new String[nclases+1];
      salida[0] = "?";
      for (int i = 1; i < nclases+1; i++) {
         salida[i] = Attributes.getOutputAttribute(0).getNominalValue(i-1);
      }
      return salida;
    }
     ...
     * @return boolean[] an array of booleans that indicates for each variable wether it is nominal or not
     */
    public boolean [] getNominales(){
      boolean [] nominales = new boolean[this.nentradas];
      for (int i = 0; i < nentradas; i++){
        nominales[i] = Attributes.getInputAttribute(i).getNumNominalValues() > 0;
      }
      return nominales;
    }
}
