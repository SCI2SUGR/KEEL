 ...
 * </p>
 */
package keel.Algorithms.SVM.NU_SVM;
...
public class svmClassifier {
    ...
    public void process() {
       ...
       try {
           ...
           paramCheck = svm.svm_check_parameter(SVMp, SVMparam);
           if (paramCheck != null){
        	   if(paramCheck.compareTo("specified nu is infeasible")==0) {
        		   System.out.print("SVM parameter error in training: ");
        		   System.out.println(paramCheck);
	        	   while(paramCheck!=null && paramCheck.compareTo("specified nu is infeasible")==0){ 
	        		   SVMparam.nu = SVMparam.nu /1.10;
	        		   System.out.println("nu adjusted to = "+ SVMparam.nu);
	        		   paramCheck = svm.svm_check_parameter(SVMp, SVMparam);
	        	   }
        	   }else{
        		   System.out.print("SVM parameter error in training: ");
...
//			the values used for regression
        ...
        try {
            ...
            for (int i = 0; i < ndatos; i++) {
                ...
                tipo = a.getType();
                if (tipo != Attribute.NOMINAL) {
                    X[i][0] = new String(String.valueOf((int) inst.
                            getAllOutputValues()[0]));
                } else {
                    X[i][0] = new String(inst.getOutputNominalValues(0));
                }
