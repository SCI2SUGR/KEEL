 ...
 * </p>
 *
 * The ADASYN algorithm is an oversampling method used to deal with the imbalanced
 * problem.
 *
...
public class ADASYN extends Metodo {
   ...
   */
  public ADASYN (String ficheroScript) {
    super (ficheroScript);
  }
  ...
  public void run () {
    ...
    Ri = new double[positives.length];
    ...
    for (i=0; i<positives.length; i++) {
    	...
    	Ri[i] = (double)cont / (double)kSMOTE;
    	sumRi += Ri[i];
    }    
    for (i=0; i<positives.length; i++) {
    	if (Ri[i] != 0)
   ...
   /*Construction of the S set from the previous vector S*/
    ...
    System.out.println("ADASYN "+ relation + " " + (double)(System.currentTimeMillis()-tiempo)/1000.0 + "s");
     ...
     */
    void interpolate (double ra[], double rb[], int na[], int nb[], boolean ma[], boolean mb[], double resS[], double resR[], int resN[], boolean resM[], String interpolation, double alpha, double mu) {
