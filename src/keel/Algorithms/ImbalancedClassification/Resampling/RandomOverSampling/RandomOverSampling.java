...
public class RandomOverSampling extends Metodo {
  /**
   * <p>
   * The Random Over Sampling algorithm is an oversampling method used to deal with the imbalanced
   * problem.
   * </p>
   */
   ...
   */  
  public RandomOverSampling (String ficheroScript) {
    super (ficheroScript);
  }
  ...
  public void run () {
   ...
   /*Construction of the S set from the previous vector S*/
    ...
    for (m=0;j<tamS; j++, m++) {
      for (l=0; l<datosTrain[0].length; l++)
        conjS[j][l] = datosTrain[overs[m]][l];
      clasesS[j] = clasesTrain[overs[m]];
    }

    System.out.println("RandomOverSampling "+ relation + " " + (double)(System.currentTimeMillis()-tiempo)/1000.0 + "s");
