 ...
 * </p>
 *
 * The ADOMS algorithm is an oversampling method used to deal with the imbalanced
 * problem.
 *
...
public class ADOMS extends Metodo {
   ...
   */  
  public ADOMS (String ficheroScript) {
    super (ficheroScript);
  }
  ...
  public void run () {
    ...
    localData = new double[datosTrain[0].length][kSMOTE+1];
    for (i=0; i<genS.length; i++) {
    	
    	for (j=0; j<datosTrain[0].length; j++) {
    		localData[j][0] = datosTrain[positives[i%positives.length]][j];
    	}
    	for (j=0; j<kSMOTE; j++) {
    		for (l=0; l<datosTrain[0].length; l++) {
        		localData[l][j+1] = datosTrain[neighbors[i%positives.length][j]][l];    			
    		}
    	}
    	
    	pca = new PCA (true, "covariance", localData);
    	resultPCA = pca.principalComponents;
    	
    	clasesGen[i] = posID;
    	nn = Randomize.Randint(0,kSMOTE-1);
    	interpolate (datosTrain[positives[i%positives.length]],datosTrain[neighbors[i%positives.length][nn]],genS[i],interpolation,alpha,mu,resultPCA[0]);
    }
   ...
   /*Construction of the S set from the previous vector S*/
    ...
    System.out.println("ADOMS "+ relation + " " + (double)(System.currentTimeMillis()-tiempo)/1000.0 + "s");
   ...
   */  
     ...
     */
	void interpolate (double ra[], double rb[], double resR[], String interpolation, double alpha, double mu, double pca[]) {
		...
		vector = new double[resR.length];
		for (i=0; i<resR.length; i++) {
			vector[i] = resR[i] - ra[i];
			magnitud += (resR[i] - ra[i])*(resR[i] - ra[i]);
		}
		...
		for (i=0; i<vector.length; i++) {
			if (vector[i] >= 0) {
				resR[i] = ra[i] + pca[i]*magnitud;				
			} else {
				resR[i] = ra[i] + pca[i]*magnitud*(-1);				
			}
		}
