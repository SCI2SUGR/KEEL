 ...
 * values. String attributes are not used by the learning schemes in Weka. They
 * can be used, for example, to store an identifier with each instance in a
 * dataset.
 * </ul>
 * </ul>
...
public class Attribute implements Copyable, Serializable {
	...
	public Attribute(String attributeName, String dateFormat) {
		...
		this(attributeName, dateFormat, new ProtectedProperties(
				new Properties()));
	}
		...
		if (attributeValues == null) {
			...
			for (int i = 0; i < attributeValues.size(); i++) {
				...
				if (((String) store).length() > STRING_COMPRESS_THRESHOLD) {
					try {
						store = new SerializedObject(attributeValues
								.elementAt(i), true);
					} catch (Exception ex) {
						System.err
								.println("Couldn't compress nominal attribute value -"
	...
	public final/* @ pure @ */boolean isReal() {

		return ((m_Type == REAL) || (m_Type == DATE));
	}
	...
	public final String toString() {
		...
		case NOMINAL:
			...
			break;
		case REAL:
			text.append(ARFF_ATTRIBUTE_REAL);
			break;
		case STRING:
	...
	/**
	 * Constructor for a real attribute with a particular index.
	 * 
	 * @param attributeName
	 *            the name for the attribute
	 * @param index
	...
	final void setValue(int index, String string) {
		...
		case STRING:
			...
			if (string.length() > STRING_COMPRESS_THRESHOLD) {
				try {
					...
					System.err
							.println("Couldn't compress string attribute value -"
									+ " storing uncompressed.");
				}
			}
		...
		default:
			throw new IllegalArgumentException(
					"Can only set values for nominal"
	 ...
	 * 
	 * @return the lower bound of the specified real range
	 */
	public final/* @ pure @ */double getLowerrealBound() {
	 ...
	 * 
	 * @return whether the lower real bound is open or not (closed)
	 */
	public final/* @ pure @ */boolean lowerrealBoundIsOpen() {
	 ...
	 * 
	 * @return the upper bound of the specified real range
	 */
	public final/* @ pure @ */double getUpperrealBound() {
	 ...
	 * 
	 * @return whether the upper real bound is open or not (closed)
	 */
	public final/* @ pure @ */boolean upperrealBoundIsOpen() {
	...
	private void setMetadata(ProtectedProperties metadata) {
		...
		if (m_Type == DATE) {
			...
			// real ordered attributes are averagable and zeropoint by
			// default
			String def;
			if (m_Type == REAL && orderString.compareTo("modulo") != 0
			...
			m_IsAveragable = (m_Metadata.getProperty("averageable", def)
					.compareTo("true") == 0);
			m_HasZeropoint = (m_Metadata.getProperty("zeropoint", def)
					.compareTo("true") == 0);
			// averagable or zeropoint implies regular
			if (m_IsAveragable || m_HasZeropoint)
				def = "true";
			m_IsRegular = (m_Metadata.getProperty("regular", def).compareTo(
					"true") == 0);
			...
			if (orderString.compareTo("symbolic") == 0)
				...
				if (m_Type == REAL || m_IsAveragable || m_HasZeropoint)
					m_Ordering = ORDERING_ORDERED;
				else
					m_Ordering = ORDERING_SYMBOLIC;
			}
		...
		if (m_IsAveragable && !m_IsRegular)
			throw new IllegalArgumentException(
					"An averagable attribute must be" + " regular");
		...
		if (m_IsAveragable && m_Ordering != ORDERING_ORDERED)
			throw new IllegalArgumentException(
					"An averagable attribute must be" + " ordered");
		...
		if (weightString != null) {
			try {
				...
				throw new IllegalArgumentException(
						"Not a valid attribute weight: '" + weightString + "'");
			}
		}

		// determine real range
		if (m_Type == REAL)
			setrealRange(m_Metadata.getProperty("range"));
	}

	/**
	 * Sets the real range based on a string. If the string is null the range
	 ...
	 * 
	 * @param rangeString
	 *            the string to parse as the attribute's real range
	 * @exception IllegalArgumentException
	 *                if the range is not valid
	 */
	// @ requires rangeString != null;
	private void setrealRange(String rangeString) {
		// set defaults
		...
		try {
			...
			if (tokenizer.ttype != tokenizer.TT_WORD)
				throw new IllegalArgumentException(
						"Expected lower bound in range," + " found: "
								+ tokenizer.toString());
			...
			if (tokenizer.ttype != tokenizer.TT_WORD)
				throw new IllegalArgumentException(
						"Expected upper bound in range," + " found: "
								+ tokenizer.toString());
			if (tokenizer.sval.compareToIgnoreCase("-inf") == 0)
			...
			if (tokenizer.ttype == ']')
				...
				throw new IllegalArgumentException(
						"Expected closing brace on range," + " found: "
								+ tokenizer.toString());

			// check for rubbish on end
			if (tokenizer.nextToken() != tokenizer.TT_EOF)
				throw new IllegalArgumentException(
						"Expected end of range string," + " found: "
								+ tokenizer.toString());

		} catch (IOException e) {
			throw new IllegalArgumentException(
					"IOException reading attribute range" + " string: "
							+ e.getMessage());
		}

		if (m_UpperBound < m_LowerBound)
			throw new IllegalArgumentException("Upper bound (" + m_UpperBound
					+ ") on real range is" + " less than lower bound ("
					+ m_LowerBound + ")!");
	}
	...
	public static void main(String[] ops) {
		...
		try {
			...
			// Create real attributes "length" and "weight"
			...
			System.out.println("\"position\" is nominal: "
					+ position.isNominal());
			System.out
					.println("\"position\" is string: " + position.isString());
