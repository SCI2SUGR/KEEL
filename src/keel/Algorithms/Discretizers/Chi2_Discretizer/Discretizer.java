...
public abstract class Discretizer {
	...
	public void buildCutPoints(InstanceSet is) {
		...
		while (inconsistencyCheck(instances) < inconsistencyThreshold) {
			round = true;
		...
		do {
			...
			for (i=0; i<realAttributes.length; i++) {
				if (realAttributes[i]) {
					...
					if (inconsistencyCheck(instances) < inconsistencyThreshold) {
						sigLvl[i] *= 0.9;
					} else {
						notMergeable[i] = true;
					}
	...
	public double inconsistencyCheck (Instance instances[]) {
		...
		for (i=0; i<data.length; i++) {
			...
			if (matches.size() > 0) {
				...
				max = countClass[0];
				for (j=1; j<countClass.length; j++) {
					if (countClass[j] > max) {
						max = countClass[j];
					}
				}
				inconsistencyCount += matches.size() + 1 - max;
			}
		}
		
		return (double)inconsistencyCount / (double)data.length;
	}
