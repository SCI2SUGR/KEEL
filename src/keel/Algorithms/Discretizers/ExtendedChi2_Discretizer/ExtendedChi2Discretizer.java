 ...
 */
	...
	public ExtendedChi2Discretizer() {
	...
	protected Vector discretizeAttribute(int attribute,int []values, Vector <Interval> intervals, double levelSig) {
		...
		while(intervals.size()>1 && !exit) {
			...
			for(int i=0;i<intervals.size()-1;i++) {
				...
				if(posMin==-1) {
					threshold = critchi(levelSig, nClasses-1);
					if (chi2 < threshold) {
						posMin=i;
						diffMin = (threshold - chi2) / Math.sqrt(2*(nClasses-1));
						negativa = false;
					}
				} else {
					threshold = critchi(levelSig, nClasses-1);
					if((threshold - chi2) < diffMin) {
						if (chi2 < threshold) {
							posMin=i;
							diffMin = (threshold - chi2) / Math.sqrt(2*(nClasses-1));
							negativa = false;
